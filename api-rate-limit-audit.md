...
> **Correction, 17 September 2026.** This audit counts where `rateLimit()` is _called_. It is not evidence that any limit is _enforced_. In production the limiter has no shared backend configured, so every count below is per serverless instance and none of the limits holds. Read this file as a coverage map only, alongside `business-docs/compliance/controls/rate-limiting-control-status.md`.
