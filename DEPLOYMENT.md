...
Subscriptions are created with a **30-day free trial** (`trial_period_days: 30`), configured in `src/constants/pricing.ts`. During the trial, `subscription.status` is `trialing`, which the webhook maps to `pro` access.
