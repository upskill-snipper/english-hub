  ...
  const validateCode = useCallback(async (raw: string): Promise<void> => {
    ...
    try {
      const res = await fetch(
        `/api/promo/validate?code=${encodeURIComponent(normalised)}`,
        { method: 'GET' },
      )
  ...
  const onRedeem = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        // so they come straight back here afterwards.
    ...
    <div className="mx-auto max-w-2xl px-4 py-12">
      <header className="mb-8 space-y-3">
        ...
        <p className="text-muted-foreground">
          Promo codes are redeemed here on the web. Once you complete
          checkout, your subscription will sync to any mobile device
          you use automatically — no further action required.
        </p>
      </header>

      {cancelled && (
        <Card className="mb-6 border-amber-300 bg-amber-50 p-4 text-sm text-amber-900">
          Checkout was cancelled. You can try again below, or return to the
          app and continue with the standard plan.
        </Card>
      )}
      ...
      <Card className="space-y-5 p-6">
        ...
        {validation?.valid && previewPrice !== null && (
          <div className="rounded-lg bg-emerald-50 p-4 text-sm text-emerald-900">
            ...
            <span className="font-semibold">{previewPrice}</span> for the
            first year. It then renews at our standard price — cancel
            anytime.
          </div>
        )}
        ...
        <Button
          type="button"
          disabled={
            !validation?.valid || isValidating || isRedeeming || code.length === 0
          }
          onClick={() => {
            void onRedeem()
          }}
      ...
      <section className="mt-10 space-y-3 text-sm text-muted-foreground">
        ...
        <ol className="list-decimal space-y-2 pl-5">
          ...
          <li>
            Your subscription is activated against your English Hub
            account immediately.
          </li>
          <li>
            If you use the mobile app, open it and pull to refresh —
            your new entitlement will appear automatically within a few
            seconds. No in-app purchase is required.
          </li>
        </ol>
      </section>
    </div>
