...
const QUESTIONS: [
  ...
  {
    id: 'board',
    prompt: 'Which exam board are you sitting?',
    helper: 'Most schools tell you this — check with your teacher if unsure.',
    options: [
      { value: 'AQA', label: 'AQA' },
...
function buildPlan(answers: StudyPlanAnswers): PlannedWeek[] {
  const weekCount = (() => {
  ...
  const TEXT_DEEPDIVE: PlannedTask = {
    title: 'Deep-dive a set text chapter or scene',
  ...
  // Hour-aware task count: 2 tasks if low hours, 3 tasks if mid, 4 if high
  const tasksPerWeek = (() => {
  ...
  for (let w = 0; w < weekCount; w++) {
    ...
    // Always include the weak-area boost in odd weeks
    ...
    const focus = (() => {
      // Final week is always exam practice; first week is foundations
...
export default function StudyPlanPage() {
  ...
  const handleAnswer = <T extends string>(value: T) => {
    ...
    const generated = buildPlan(fullAnswers)
  ...
  // Pre-mount: render skeleton-ish to avoid hydration mismatch
  if (!mounted) {
    return (
        ...
        <section className="rounded-2xl border border-border/60 bg-card p-6 sm:p-8">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="rounded-xl border border-border/40 bg-background/50 p-4">
              ...
              <p className="mt-1 text-sm font-semibold text-foreground">{meta.board}</p>
            </div>
            <div className="rounded-xl border border-border/40 bg-background/50 p-4">
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <div className="rounded-xl border border-border/40 bg-background/50 p-4">
              ...
              <p className="mt-1 text-sm font-semibold text-foreground">
                {QUESTIONS[4].options.find((o) => o.value === meta.hoursPerWeek)?.label}
              </p>
            </div>
      ...
      <div className="space-y-8 pb-16">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-body-sm text-muted-foreground">
            Answer five quick questions and we will generate a week-by-week revision plan tailored
            to your exam date, target grade, and weakest area. Your plan is saved on this device so
