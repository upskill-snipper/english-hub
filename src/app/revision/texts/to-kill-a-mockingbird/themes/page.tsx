...
export const metadata: Metadata = {
  ...
  description:
    'Detailed theme analysis for To Kill a Mockingbird by Harper Lee: Racial Injustice, Moral Education, Loss of Innocence, Courage, Class and Empathy.',
  alternates: {
    canonical: 'https://theenglishhub.app/revision/texts/to-kill-a-mockingbird/themes',
  },
}
...
export default async function ToKillAMockingbirdThemesPage() {
  ...
  const backLabel = (await tx('rev.texts.common.back_to_text')).replace(
    '{text}',
    'To Kill a Mockingbird',
  )
  const overviewLabel = await tx('rev.texts.common.overview')
  const howLeeLabel = await tx('rev.texts2.tkam.themes.how_lee')
    ...
    <div className="space-y-10 pb-16">
      <BreadcrumbJsonLd
        items={[
          ...
          {
            name: 'Themes',
            url: 'https://theenglishhub.app/revision/texts/to-kill-a-mockingbird/themes',
          },
        ]}
      />
      {/* Hero */}
        ...
        <div className="relative">
          ...
          <div className="mb-4 flex flex-wrap items-center gap-2">
            <Badge variant="secondary">
              ...
              {await tx('rev.texts2.common.modern_text_novel')}
            </Badge>
          ...
          <p className="mt-4 max-w-2xl text-body-md text-muted-foreground">
            {await tx('rev.texts2.tkam.themes.intro')}
          </p>
        </div>
      </section>
          ...
          {THEMES.map((t) => (
            <Card key={t.title}>
              ...
              <CardContent className="space-y-5">
                ...
                <div>
                  <h3 className="mb-1 text-sm font-semibold text-foreground">{howLeeLabel}</h3>
      ...
      <section className="flex flex-wrap gap-3">
        ...
        >
          {await tx('rev.texts.common.characters')} <ArrowRight className="size-3.5" />
        </Button>
        <Button
          variant="outline"
          size="sm"
        ...
        >
          {await tx('rev.texts2.common.key_quotes_sc')} <ArrowRight className="size-3.5" />
        </Button>
        <Button
          variant="outline"
          size="sm"
        ...
        >
          {await tx('rev.texts.common.context')} <ArrowRight className="size-3.5" />
        </Button>
      </section>
