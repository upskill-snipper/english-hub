...
const essayPlans: EssayPlan[] = [
  ...
  /* ── Essay 4: Relationship between Jekyll and Hyde ──────────── */
  {
    ...
    introduction:
      'Open with context: the novella was famously inspired by a vivid dream and drafted at remarkable speed (Stevenson reportedly burned his first draft and rewrote it within weeks), and its compulsive composition mirrors the uncontrollable nature of Jekyll\'s relationship with Hyde. Introduce the thesis that the "relationship" is really a war within a single self, and that Stevenson uses it to explore Victorian anxieties about repression, addiction, and loss of control.',
    paragraphs: [
      {
        pointLabel: 'Point 1',
      ...
      },
      {
        ...
        context:
          "A Freudian critical lens (developed decades after Stevenson, but commonly applied retrospectively) reads Jekyll's attempt to separate the id (instinctual desire) from the superego (moral conscience) as only empowering the id - the return of the repressed. Examiners reward this as a critical reading, not as authorial intention.",
      },
      {
        pointLabel: 'Point 4',
        ...
        <div className="relative">
          ...
          <div className="mb-4 flex flex-wrap items-center gap-2">
            <Badge variant="secondary">
              <Feather className="me-1 size-3 text-violet-400" />
      ...
      {essayPlans.map((ep, essayIdx) => (
        <section key={ep.id} id={`essay-${ep.id}`} className="scroll-mt-8 space-y-5">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-heading-lg font-heading text-foreground">{ep.question}</h2>
          </div>
          ...
          {essayIdx < essayPlans.length - 1 && <div className="border-t border-border/40 pt-4" />}
        </section>
      ))}
