...
const MOCK_WEAKNESSES: Record<
  string,
  { area: string; description: string; recommendedPath: string }[]
> = {
  s1: [
    {
      area: 'Grammar',
      description: 'Sentence structure and punctuation need improvement',
      recommendedPath: 'Grammar Foundations module + Punctuation Practice Pack',
    },
    {
      area: 'Creative Writing',
      description: 'Vocabulary variety and descriptive techniques could be expanded',
      recommendedPath: 'Creative Writing Masterclass + Vocabulary Builder',
    },
    {
      area: 'Essay Structure',
      description: 'Conclusions sometimes lack strength',
      recommendedPath: 'Essay Conclusions Workshop + Model Answer Analysis',
    },
  ],
  s2: [
    {
      area: 'Analysis',
      description: 'Needs to develop deeper analytical skills with textual evidence',
      recommendedPath: 'Analysis Skills module + Quote Integration Practice',
    },
    {
      area: 'Grammar',
      description: 'Common errors in tense consistency',
      recommendedPath: 'Tense Consistency Workshop + Grammar Drills',
    },
    {
      area: 'Reading Comprehension',
      description: 'Inference skills need development',
      recommendedPath: 'Inference & Deduction Practice + Comprehension Pack',
    },
  ],
}

const MOCK_GRADE_COMPARISON: Record<
  string,
  { subject: string; workingAt: string; predicted: string; target: string }[]
> = {
  s1: [
    ...
    <div className="card">
      ...
      <div className="relative h-48">
        ...
        <div className="absolute left-0 top-0 bottom-6 flex flex-col justify-between text-xs text-muted-foreground w-8">
          ...
          <span>
            {formatValue ? formatValue(Math.round((max + min) / 2)) : Math.round((max + min) / 2)}
          </span>
  ...
  return (
    <div className="card">
      <h3 className="text-sm font-semibold text-foreground mb-4">
        {tx('dashboard.parent_progress_detail.strengths_radar')}
      </h3>
      <div className="space-y-4">
        {skills.map((skill) => {
          const barColor =
            skill.value >= 75 ? 'bg-success' : skill.value >= 60 ? 'bg-accent' : 'bg-warn'
          return (
            <div key={skill.label}>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-primary sm:text-3xl">
            {tx('dashboard.parent_progress_detail.title')}
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {tx('dashboard.parent_progress_detail.subtitle')} {student.name}.
          </p>
        </div>
      ...
      <section aria-labelledby="score-trends-heading">
        <h2 id="score-trends-heading" className="sr-only">
          Score Trends
        </h2>
        <LineChart
          data={scoreTrends}
          valueKey="score"
          label={tx('dashboard.parent_progress_detail.score_trend_label')}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section aria-labelledby="essays-per-week-heading">
          <h2 id="essays-per-week-heading" className="sr-only">
            Essays Per Week
          </h2>
          <LineChart
            data={essaysPerWeek}
            valueKey="count"
            label={tx('dashboard.parent_progress_detail.essays_per_week_label')}
            color="hsl(var(--chart-2))"
          />
        </section>

        <section aria-labelledby="time-spent-heading">
          <h2 id="time-spent-heading" className="sr-only">
            Time Spent Studying
          </h2>
          <LineChart
            data={timeSpent}
            valueKey="minutes"
            label={tx('dashboard.parent_progress_detail.time_spent_label')}
      ...
      <section aria-labelledby="weaknesses-heading">
        <h2 id="weaknesses-heading" className="text-lg font-semibold text-foreground">
          {tx('dashboard.parent_progress_detail.areas_improvement')}
        </h2>
            ...
            <div key={idx} className="card border-l-4 border-l-warn">
              ...
              <div className="mt-3 rounded-lg bg-primary/10 px-4 py-3">
                <p className="text-xs font-semibold uppercase tracking-wider text-primary">
                  {tx('dashboard.parent_progress_detail.recommended_path')}
                </p>
      ...
      <section aria-labelledby="grades-comparison-heading">
        <h2 id="grades-comparison-heading" className="text-lg font-semibold text-foreground">
          {tx('dashboard.parent_progress_detail.grades_heading')}
        </h2>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  >
                    {tx('dashboard.parent_progress_detail.col_subject')}
                  </th>
                  <th
                    scope="col"
                    className="pb-2 pr-4 text-left text-xs font-medium text-muted-foreground"
                  >
                    {tx('dashboard.parent_progress_detail.col_working_at')}
                  </th>
                  <th
                    scope="col"
                    className="pb-2 pr-4 text-left text-xs font-medium text-muted-foreground"
                  >
                    {tx('dashboard.parent_progress_detail.col_predicted')}
                  </th>
                  <th
                    scope="col"
                    className="pb-2 pr-4 text-left text-xs font-medium text-muted-foreground"
                  >
                    {tx('dashboard.parent_progress_detail.col_target')}
                  </th>
                  <th
                    scope="col"
                    className="pb-2 text-left text-xs font-medium text-muted-foreground"
                  >
                    {tx('dashboard.parent_progress_detail.col_status')}
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {gradeComparison.map((row, idx) => {
                  ...
                  const diff = p - t
                  let statusLabel = tx('dashboard.parent_progress_detail.status_on_track')
                  let statusClass = 'bg-success-50 text-success-700'
                  if (diff < 0) {
                    statusLabel = `${Math.abs(diff)} grade${Math.abs(diff) > 1 ? 's' : ''} below target`
                    statusClass =
                      diff <= -2 ? 'bg-warn-50 text-warn-700' : 'bg-yellow-500/10 text-yellow-700'
                  } else if (diff > 0) {
                    statusLabel = tx('dashboard.parent_progress_detail.status_above_target')
                  ...
                  return (
                    <tr key={idx}>
                      ...
                      <td
                        className={`py-2.5 pr-4 font-bold ${gradeColour(row.workingAt, row.target)}`}
                      >
                        Grade {row.workingAt}
                      </td>
                      <td
                        className={`py-2.5 pr-4 font-bold ${gradeColour(row.predicted, row.target)}`}
                      >
                        Grade {row.predicted}
                      </td>
