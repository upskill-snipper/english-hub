...
Somewhere ahead, a chain-link fence sang in the wind - a thin, metallic hymn that raised the hairs on my forearms. It was the sound of a place that remembered what it used to be.`,
    annotations: [
      ...
      },
      {
        ...
        example: '"rusted", "cracked", "shattered", "peeling"',
        effect: 'Consistent imagery reinforces the theme of neglect and the passage of time',
      },
      {
      ...
      },
      {
        ...
        example: '"nature reclaiming", "a place that remembered"',
        effect:
          'Gives agency to non-human elements, suggesting the fairground has its own consciousness',
      },
      {
        technique: 'Varied sentence length',
        example: '"A ticket booth, its glass shattered..." (fragment) vs complex opening sentence',
        effect: 'Fragments create pace and impact; longer sentences build atmosphere',
      },
      {
        technique: 'Sensory detail',
        example:
          'sight (bruised sky), taste (iron), touch (hairs on forearms), sound (metallic hymn)',
        effect: 'Engages multiple senses to immerse the reader in the setting',
      },
    ],
  },
]
...
const WRITING_TASKS = [
  {
    ...
    success: [
      ...
      'At least one simile or metaphor using non-visual imagery',
    ],
  },
  {
    ...
    time: '20 minutes',
    instruction:
      'Rewrite the model text from the perspective of the carousel horse. Use first-person narration. How does the horse experience time passing? What does it remember?',
    success: [
      'Consistent first-person voice',
      'Evidence of time passing (then vs now)',
      'At least one flashback to when the fairground was active',
      'Personification that feels natural, not forced',
    ],
  },
  {
    ...
    instruction:
      "'Write a description suggested by this picture.' [Image: an empty playground at dusk]. You should write between 350-500 words. Use the techniques from the model text to create atmosphere.",
    success: [
      'Clear sense of atmosphere from the opening line',
      'Varied sentence structures (including at least 2 fragments)',
      'Extended metaphor or motif running through the piece',
      'Sensory details beyond just visual',
      'Effective ending that echoes or develops the opening image',
    ],
  },
]
...
const TECHNIQUE_TOOLKIT = [
  {
    ...
    description: 'Weather/setting reflecting mood',
    example: '"The sky bruised purple as the argument escalated"',
    when: 'Opening paragraphs, transitions between moods',
  },
  {
    name: 'In medias res',
    description: 'Starting in the middle of action',
    example: '"I ran. Behind me, the sirens wailed."',
    when: 'Narrative openings to create instant tension',
  },
  {
    name: 'Cyclical structure',
    description: 'Ending echoes the beginning',
    example: 'Open with rain on a window → end with rain clearing',
    when: 'To create a satisfying, literary structure',
  },
  {
    name: 'One-sentence paragraph',
    description: 'Isolated sentence for dramatic effect',
    example: '"She didn\'t come back."',
    when: 'After a build-up, to deliver impact or a turning point',
  },
  {
    name: 'Zooming in',
    description: 'Moving from wide shot to close-up detail',
    example: 'City skyline → street → doorway → cracked paint on the door handle',
    when: "Descriptive writing to guide the reader's eye",
  },
  {
    name: 'Semantic field',
    description: 'Related words creating a pattern of meaning',
    example: 'Words of confinement: "cage", "trapped", "bars", "locked"',
    when: 'To reinforce a theme or mood throughout',
  },
]
...
export default async function CreativeWritingPage() {
  ...
  const [
    ...
    pageSubtitle,
    annotationsTitle,
    techniqueToolkitTitle,
    writingTasksTitle,
    successCriteriaLabel,
  ] = await Promise.all([
    t('dashboard.creative_writing.back'),
    t('dashboard.creative_writing.title'),
    t('dashboard.creative_writing.subtitle'),
    t('dashboard.creative_writing.annotations_title'),
    t('dashboard.creative_writing.technique_toolkit_title'),
    t('dashboard.creative_writing.writing_tasks_title'),
    t('dashboard.creative_writing.success_criteria'),
  ])
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      ...
      {/* Annotations */}
      <section className="card">
        <h2 className="text-lg font-semibold text-foreground mb-4">{annotationsTitle}</h2>
      ...
      {/* Technique Toolkit */}
      <section className="card">
        <h2 className="text-lg font-semibold text-foreground mb-4">{techniqueToolkitTitle}</h2>
        <div className="grid gap-3 sm:grid-cols-2">
          {TECHNIQUE_TOOLKIT.map((tech, i) => (
            <div key={i} className="rounded-lg border p-3">
              ...
              <p className="text-xs text-muted-foreground mt-1.5 italic bg-background rounded p-1.5">
                {tech.example}
              </p>
      ...
      {/* Writing Tasks */}
      <section>
        <h2 className="text-lg font-semibold text-foreground mb-4">{writingTasksTitle}</h2>
