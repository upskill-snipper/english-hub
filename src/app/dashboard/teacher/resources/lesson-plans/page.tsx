    ...
    lessons: [
      {
        ...
        plenary:
          "Exit ticket: 'What impression does Priestley create of the Birling family in the opening of the play? Use one quote to support your answer.'",
        differentiation:
          "Scaffolded quote analysis frame for lower-ability; extension question on dramatic irony of Mr Birling's speeches for higher-ability.",
        resources: [
          'Context presentation slides',
          'Act 1 extract (printed)',
      ...
      },
      {
        ...
        plenary:
          'Peer assessment using the AQA marking guide criteria (displayed on board). Students give one WWW and one EBI.',
        differentiation:
          'Model PEEL paragraph provided for lower-ability; higher-ability attempt a third paragraph comparing to Mr Birling.',
        resources: [
          'Mini whiteboards',
          'Comparison table handout',
          'AQA marking guide extract',
          'PEEL frame',
        ],
      },
      {
        ...
        plenary:
          "Share one 'golden sentence' from their essay plan with the class. Peer vote on strongest opening.",
        differentiation:
          'Pre-selected quotes with sentence starters for lower-ability; higher-ability select their own quotes and consider alternative interpretations.',
        resources: [
          'Venn diagram template',
          'Quote cards (6 per group)',
      ...
      },
      {
        ...
        plenary:
          'Self-assessment against AQA band descriptors. Students identify which band their response sits in and set a target for improvement.',
        differentiation:
          'Writing frame with topic sentences for lower-ability; higher-ability write under timed conditions without a frame.',
        resources: [
          'Responsibility Grid handout',
          'AQA band descriptors (student-friendly version)',
          'Timer display',
        ],
      },
      {
        ...
        plenary:
          'Gallery walk: Students read 2 other essays and leave a positive comment on a Post-it note.',
        differentiation:
          'Extract provided with key quotes highlighted for lower-ability; higher-ability given the extract without annotations.',
        resources: [
          'Extract handout',
          'Essay answer booklets',
          'Post-it notes for gallery walk',
          'AQA marking guide for self-marking',
        ],
      },
    ],
...
export default async function LessonPlansPage() {
  ...
  const [
    ...
    t('dashboard.lesson_plans.back'),
    t('dashboard.lesson_plans.overview_title'),
    t('dashboard.lesson_plans.objectives_title'),
    t('dashboard.lesson_plans.starter_label'),
    t('dashboard.lesson_plans.main_activities_label'),
    t('dashboard.lesson_plans.plenary_label'),
    t('dashboard.lesson_plans.differentiation_label'),
    t('dashboard.lesson_plans.resources_label'),
    t('dashboard.lesson_plans.assessment_title'),
    t('dashboard.lesson_plans.cross_curricular_title'),
  ])
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      ...
      <section className="card">
        <h2 className="text-lg font-semibold text-foreground mb-2">{overviewTitle}</h2>
        <p className="text-sm text-muted-foreground">{plan.overview}</p>
      </section>

      {/* Learning Objectives */}
      <section className="card">
        <h2 className="text-lg font-semibold text-foreground mb-3">{objectivesTitle}</h2>
      ...
      {plan.lessons.map((lesson) => (
        <section key={lesson.number} className="card space-y-4">
          ...
          <div>
            <h3 className="text-sm font-semibold text-foreground mb-1">{starterLabel}</h3>
            <p className="text-sm text-muted-foreground">{lesson.starter}</p>
          </div>

          <div>
            <h3 className="text-sm font-semibold text-foreground mb-2">{mainActivitiesLabel}</h3>
          ...
          <div>
            <h3 className="text-sm font-semibold text-foreground mb-1">{plenaryLabel}</h3>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-lg bg-amber-500/10 p-3">
              <h3 className="text-sm font-semibold text-amber-700 mb-1">{resourcesLabel}</h3>
      ...
      <section className="card">
        <h2 className="text-lg font-semibold text-foreground mb-3">{assessmentTitle}</h2>
      ...
      <section className="card">
        <h2 className="text-lg font-semibold text-foreground mb-3">{crossCurricularTitle}</h2>
        <ul className="space-y-1.5">
