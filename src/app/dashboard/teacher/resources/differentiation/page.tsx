...
const DIFFERENTIATED_TASK = {
  ...
  gold: {
    ...
    outcome:
      'Extended analytical response with structural analysis, alternative interpretations, and contextual links (WHAT + HOW + WHY + SO WHAT)',
  },
}
...
export default async function DifferentiationPage() {
  const [
    ...
    t('dashboard.differentiation.back'),
    t('dashboard.differentiation.title'),
    t('dashboard.differentiation.subtitle'),
    t('dashboard.differentiation.diff_by_ability'),
    t('dashboard.differentiation.lower_ability'),
    t('dashboard.differentiation.middle_ability'),
    t('dashboard.differentiation.higher_ability'),
    t('dashboard.differentiation.example_task_title'),
    t('dashboard.differentiation.pp_send_title'),
  ])
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      ...
      <section>
        <h2 className="text-lg font-semibold text-foreground mb-4">{diffByAbilityTitle}</h2>
            ...
            <div key={strat.category} className="card">
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                <div className="rounded-lg bg-amber-500/10 p-3">
                  <p className="text-xs font-bold text-amber-700 mb-2">{lowerAbility}</p>
                ...
                <div className="rounded-lg bg-emerald-500/10 p-3">
                  ...
                  <ul className="space-y-1.5">
                    {strat.higher.map((item, i) => (
                      <li key={i} className="text-xs text-emerald-700 dark:text-emerald-300">
                        {item}
                      </li>
                    ))}
                  </ul>
      ...
      <section className="card">
        <h2 className="text-lg font-semibold text-foreground mb-2">{exampleTaskTitle}</h2>
      ...
      <section>
        <h2 className="text-lg font-semibold text-foreground mb-4">{ppSendTitle}</h2>
