    ...
    images: [
      {
        ...
        width: 1200,
        height: 630,
        alt: 'Macbeth Key Quotes - Edexcel IGCSE Literature',
      },
    ],
  },
  alternates: {
...
const QUOTES = [
  ...
  {
    quote: '"All my pretty chickens and their dam at one fell swoop"',
    speaker: 'Macduff (4.3)',
    ...
    <div className="min-h-screen bg-background">
      <BreadcrumbJsonLd
        items={[
          ...
          {
            name: 'Key Quotations',
            url: 'https://theenglishhub.app/igcse/edexcel/shakespeare/macbeth/quotes',
          },
        ]}
      />
      {/* ── Hero ─────────────────────────────────────────────── */}
      <section className="border-b border-border bg-gradient-to-b from-primary/[0.06] to-transparent px-4 py-16 sm:py-20">
        <div className="mx-auto max-w-3xl text-center">
          ...
          >
            &larr; {await t('igcse.page.macbeth_hub')}
          </Link>
      ...
      <div className="mx-auto max-w-5xl px-4 py-12 sm:py-16">
        <div className="rounded-xl bg-muted p-6">
          <h2 className="text-lg font-bold text-foreground">How to revise these</h2>
          <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
            <li className="flex gap-2">
              <span className="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-primary" />
              Learn short, embeddable fragments &mdash; 3&ndash;6 words you can drop inside a
              sentence.
            </li>
            <li className="flex gap-2">
              <span className="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-primary" />
              For each quote, note at least <strong>two themes</strong> it can support.
            </li>
            <li className="flex gap-2">
              <span className="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-primary" />
              Practise the single-word zoom: pick one word and explain its effect in two or three
              sentences.
            </li>
            <li className="flex gap-2">
              <span className="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-primary" />
        ...
        <section className="mt-10 space-y-4">
          {QUOTES.map((q, i) => (
            <div key={i} className="rounded-xl border border-border bg-card p-5 shadow-md sm:p-6">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex-1">
                  <p className="text-base italic font-semibold text-foreground">{q.quote}</p>
                  <p className="mt-1 text-xs font-medium text-primary">&mdash; {q.speaker}</p>
