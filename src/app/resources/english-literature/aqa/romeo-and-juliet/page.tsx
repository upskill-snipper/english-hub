  ...
  assessmentObjectives: [
    {
      ...
      description:
        'Read, understand and respond to texts. Use textual references to support interpretations.',
      marks: '12 marks',
...
export default async function RomeoAndJulietPage() {
  return (
    <>
      ...
      <section className="border-b bg-gradient-to-b from-primary/[0.06] to-transparent px-4 py-16 sm:py-20">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-lg text-muted-foreground">
            William Shakespeare &middot; Written c. 1594&ndash;96 &middot; Tragedy
          </p>
        </div>
      </section>
      ...
      {/* ── AQA Exam Format ────────────────────────────────────────── */}
      <section id="exam-format" className="mx-auto max-w-5xl px-4 py-12 sm:py-16">
        <h2 className="text-2xl font-bold text-foreground">AQA Exam Format: What to Expect</h2>
        <p className="mt-2 text-muted-foreground">
          Understanding the exam structure is essential for maximising your marks. Romeo and Juliet
          appears in <strong>{aqaExamFormat.paper}</strong>,{' '}
        ...
        <div className="mt-6 rounded-xl border border-border bg-card p-6 shadow-md">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <p className="mt-1 text-lg font-bold text-foreground">{aqaExamFormat.marks}</p>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-accent">
                Recommended Time
              </p>
              <p className="mt-1 text-lg font-bold text-foreground">{aqaExamFormat.time}</p>
            </div>
          </div>
          <div className="mt-4">
            <p className="text-xs font-semibold uppercase tracking-wider text-accent">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          {aqaExamFormat.assessmentObjectives.map((ao) => (
            <div key={ao.code} className="rounded-xl border border-border bg-card p-5 shadow-md">
              <div className="flex items-baseline gap-3">
                <span className="rounded-full bg-primary px-3 py-1 text-xs font-bold text-white">
                  {ao.code}
                </span>
                <span className="text-sm font-semibold text-foreground">{ao.marks}</span>
              </div>
              <p className="mt-2 text-sm text-muted-foreground leading-relaxed">{ao.description}</p>
              <p className="mt-2 text-sm text-accent font-medium leading-relaxed">Tip: {ao.tip}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="plot" className="bg-muted px-4 py-12 sm:py-16">
        <div className="mx-auto max-w-5xl">
          <h2 className="text-2xl font-bold text-foreground">Plot Summary: Act by Act</h2>
          <p className="mt-2 text-muted-foreground">
            The action of Romeo and Juliet takes place over just five days, creating a breathless
            pace that mirrors the intensity of the lovers&apos; passion.
          </p>
                  ...
                  {act.scenes.map((scene) => (
                    <div key={scene.scene} className="rounded-lg border border-border bg-card p-4">
                      <p className="text-xs font-semibold text-accent uppercase tracking-wider">
                        {scene.scene.includes('Prologue') ? scene.scene : `Scene ${scene.scene}`}
                      </p>
                      <p className="mt-1 text-sm text-muted-foreground leading-relaxed">
                        {scene.summary}
                      </p>
      ...
      <section id="characters" className="mx-auto max-w-5xl px-4 py-12 sm:py-16">
        ...
        <p className="mt-2 text-muted-foreground">
          Understanding characters as constructs &mdash; Shakespeare made deliberate choices about
          how to present each character to convey meaning.
        </p>

        <div className="mt-8 space-y-8">
          {characters.map((char) => (
            <div key={char.name} className="rounded-xl border border-border bg-card p-6 shadow-md">
              <div className="flex flex-wrap items-baseline gap-3">
                <h3 className="text-xl font-bold text-foreground">{char.name}</h3>
                <span className="text-sm text-accent font-medium">{char.role}</span>
              </div>
              <p className="mt-3 text-sm text-muted-foreground leading-relaxed">
                {char.description}
              </p>
              <div className="mt-4">
                <h4 className="text-sm font-semibold text-foreground">Key Points for Analysis</h4>
      ...
      <section id="themes" className="bg-muted px-4 py-12 sm:py-16">
        <div className="mx-auto max-w-5xl">
          <h2 className="text-2xl font-bold text-foreground">Key Themes &mdash; AQA Focus</h2>
          <p className="mt-2 text-muted-foreground">
            Themes are the big ideas Shakespeare explores through the plot, characters, and language
            of the play.
          </p>

          <div className="mt-8 space-y-8">
            {themes.map((theme) => (
              <div key={theme.title} className={`rounded-xl border p-6 ${theme.colour}`}>
      ...
      <section id="quotations" className="bg-primary/10 px-4 py-12 sm:py-16">
        <div className="mx-auto max-w-5xl">
          <h2 className="text-2xl font-bold text-foreground">Key Quotations with Analysis</h2>
          <p className="mt-2 text-muted-foreground">
            Learn these quotations and their analysis. In the exam, embed short phrases rather than
            copying entire quotes. AQA rewards analysis of individual words and techniques
            (analysing language and structure).
          </p>

          <div className="mt-8 space-y-4">
            {keyQuotations.map((q, i) => (
              <div key={i} className="rounded-xl bg-card p-5 shadow-md border border-border">
                <p className="text-base font-semibold italic text-primary">
                  &ldquo;{q.quote}&rdquo;
                </p>
                <p className="mt-1 text-xs text-accent font-medium">{q.speaker}</p>
      ...
      <section id="context" className="mx-auto max-w-5xl px-4 py-12 sm:py-16">
        <h2 className="text-2xl font-bold text-foreground">Historical and Social Context</h2>
        <p className="mt-2 text-muted-foreground">
          Relating to context (what markers look for) is worth 6 marks. Integrate it into your
          analysis rather than writing separate &quot;context paragraphs.&quot;
        </p>

        <div className="mt-8 grid gap-6 sm:grid-cols-2">
          {contextPoints.map((ctx) => (
            <div key={ctx.title} className="rounded-xl border border-border p-5">
              <h3 className="font-bold text-primary">{ctx.title}</h3>
              <p className="mt-2 text-sm text-muted-foreground leading-relaxed">{ctx.detail}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ── AQA Command Words ──────────────────────────────────── */}
      <section id="command-words" className="bg-muted px-4 py-12 sm:py-16">
        <div className="mx-auto max-w-5xl">
          <h2 className="text-2xl font-bold text-foreground">
            AQA Command Words &mdash; What They Really Mean
          </h2>
          <p className="mt-2 text-muted-foreground">
            Understanding the question wording is crucial. Here are the key phrases AQA uses and
            what they require from you.
          </p>

          <div className="mt-8 space-y-4">
            {commandWords.map((cw) => (
              <div key={cw.word} className="rounded-xl border border-border bg-card p-5 shadow-md">
                <p className="font-bold text-primary italic">&ldquo;{cw.word}&rdquo;</p>
                <p className="mt-2 text-sm text-muted-foreground leading-relaxed">{cw.meaning}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="essays" className="mx-auto max-w-5xl px-4 py-12 sm:py-16">
        <h2 className="text-2xl font-bold text-foreground">Essay Planning: Sample AQA Questions</h2>
        <p className="mt-2 text-muted-foreground">
          Practise planning essays in 5 minutes. These reflect the style of AQA Paper 1 Section A
          questions. Remember: start with the extract, then move to the wider play.
        </p>

        <div className="mt-8 space-y-6">
          {essayQuestions.map((eq, i) => (
            <div key={i} className="rounded-xl border border-border bg-card p-6 shadow-md">
