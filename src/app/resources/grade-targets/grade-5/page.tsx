...
const SKILLS_CHECKLIST = [
  ...
  {
    skill: 'I can use subject terminology correctly (simile, metaphor, etc.)',
    category: 'Reading',
  },
  ...
  {
    skill: 'I can write a clear thesis statement that answers the question',
    category: 'Literature',
  },
  {
    skill: 'I can write PEEL paragraphs (Point, Evidence, Explanation, Link)',
    category: 'Literature',
  },
...
const SPECIFIC_TIPS = [
  {
    ...
    detail:
      'What is the writer saying? How are they saying it (technique/language)? Why -- what effect does this create for the reader?',
  },
  {
    tip: 'Front-load your paragraphs',
    detail:
      "Start each paragraph with a clear, direct point that answers the question. Don't build up to your point -- state it immediately.",
  },
  {
    tip: 'Learn 5 context points per text',
    detail:
      "You don't need to be a history expert. Learn 5 key context points for each Literature text and practise linking them to the writer's message.",
  },
  {
    tip: 'Plan for 5 minutes, write for 25',
    detail:
      "A 5-minute plan saves time overall. Jot down your key points, quotations, and the order you'll write them in. Planned responses are always better.",
  },
  {
    tip: 'Read the mark scheme',
    detail:
      "Look at what examiners actually reward. Grade 5 descriptors use words like 'clear,' 'relevant,' and 'appropriate' -- make sure your responses match these.",
  },
  {
    tip: 'Check your work with CUPS',
    detail:
      'Capitals, Understanding (does it make sense?), Punctuation, Spelling. Spend the last 2-3 minutes of every answer checking for errors.',
  },
]
  ...
  return (
    <>
      ...
      <section className="border-b bg-gradient-to-b from-primary/[0.06] to-transparent px-4 py-16 sm:py-20">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-lg text-muted-foreground">
            A Grade 5 is a strong pass that shows clear understanding and competent skills. Here is
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        <h2 className="text-2xl font-bold text-foreground">What examiners expect at Grade 5</h2>
        <p className="mt-2 text-muted-foreground">
          The key word at Grade 5 is <strong>clear</strong>. Examiners want to see clear
          understanding, clear explanations, and clear communication. You don&apos;t need to be
          brilliant -- you need to be consistently competent.
        </p>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-3">
          {EXAMINER_EXPECTATIONS.map((section) => (
            ...
            >
              <h3 className="text-lg font-bold text-foreground">{section.area}</h3>
      ...
      <section className="bg-muted px-4 py-12 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <h2 className="text-2xl font-bold text-foreground">Grade 5 Skills Checklist</h2>
          <p className="mt-2 text-muted-foreground">
            Tick off the skills you are confident with. Any gaps? Those are your revision
            priorities.
          </p>
          ...
          {['Reading', 'Writing', 'Literature'].map((category) => (
            <div key={category} className="mt-6">
              ...
              <div className="mt-2 space-y-2">
                {SKILLS_CHECKLIST.filter((s) => s.category === category).map((item) => (
                  <label
                    key={item.skill}
      ...
      {/* Common weaknesses */}
      <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        <h2 className="text-2xl font-bold text-foreground">Common weaknesses at this level</h2>
        <p className="mt-2 text-muted-foreground">
          These are the mistakes that keep students at Grade 4 when they could be reaching a 5.
          Recognise any?
        </p>
      ...
      {/* Example responses */}
      <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        <h2 className="text-2xl font-bold text-foreground">Example Grade 5 Responses</h2>
        <p className="mt-2 text-muted-foreground">
          See what Grade 5 work actually looks like, with annotations explaining what works and how
          to improve.
        </p>
          ...
          {EXAMPLE_RESPONSES.map((example) => (
            ...
            >
              <div className="border-b border-border bg-muted px-6 py-4">
                ...
                <p className="mt-2 font-semibold text-foreground">{example.question}</p>
                {example.extract && (
                  <p className="mt-2 text-sm italic text-muted-foreground">{example.extract}</p>
                )}
              </div>
      ...
      <section className="bg-muted px-4 py-12 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <h2 className="text-2xl font-bold text-foreground">Grade 5 Tips</h2>
          <p className="mt-2 text-muted-foreground">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {SPECIFIC_TIPS.map((item) => (
              <div key={item.tip} className="rounded-xl border border-border bg-card p-5 shadow-md">
