...
const QUICK_STATS = [
  ...
  { label: "Assessment Tools", value: "4", color: "bg-[#2E86C1]" },
    ...
    <>
      ...
      <section className="mx-auto -mt-8 max-w-5xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          {QUICK_STATS.map((stat) => (
            ...
            >
              <p className="text-3xl font-bold text-[#1A5276]">{stat.value}</p>
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2">
          {SECTIONS.map((section) => {
            ...
            return (
              <div
                key={section.title}
                className={`group relative flex flex-col rounded-xl border border-gray-200 bg-white p-6 shadow-sm transition ${
                  section.comingSoon
              ...
              >
                ...
                {!section.comingSoon && !section.external && (
                  <Link
                    href={section.href}
                    className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-[#2E86C1] group-hover:text-[#1A5276] transition-colors"
                  >
                    Explore
      ...
      <section className="bg-gray-50 px-4 py-12 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <h2 className="text-2xl font-bold text-gray-900">Exam Board Specifications</h2>
          <p className="mt-2 text-gray-600">
          ...
          <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {SPEC_LINKS.map((board) => (
              <div
                key={board.board}
                className={`rounded-xl border-l-4 ${board.color} bg-white p-5 shadow-sm`}
              >
                <h3 className="font-bold text-gray-900">{board.board}</h3>
                  ...
                  {board.links.map((link) => (
                    <li key={link.label}>
                      <a
                        ...
                        className="inline-flex items-center gap-1 text-sm text-[#2E86C1] hover:text-[#1A5276] hover:underline transition-colors"
                      >
                        {link.label}
