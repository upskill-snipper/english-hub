...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: profile, error: profileError } = await admin
      .from('profiles')
      .select(
        'id, email, full_name, role, year_group, exam_board, school_name, subscription_status, subscription_end_date, date_of_birth, created_at, updated_at',
      )
      .eq('id', user.id)
      .single()
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { data: updatedProfile, error: updateError } = await admin
      ...
      .eq('id', user.id)
      .select(
        'id, email, full_name, role, year_group, exam_board, school_name, subscription_status',
      )
      .single()
