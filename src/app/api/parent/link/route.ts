...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!result) {
      return NextResponse.json(
        {
          error: 'Invalid or expired invite code. Please ask your child to generate a new one.',
        },
        { status: 400 },
      )
    }
    ...
    const hasSubscription = await verifyStudentHasActiveSubscription(student.id)
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const {
      data: { user: authUserDel },
      error: authErrorDel,
    } = await supabaseDel.auth.getUser()
    if (authErrorDel || !authUserDel) {
      return NextResponse.json({ error: 'Authentication required.' }, { status: 401 })
    }

    const dbUserDelBySupabaseId = await prisma.user.findUnique({
      where: { supabaseUserId: authUserDel.id },
      select: {
        id: true,
        role: true,
        accountStatus: true,
    ...
    const dbUserDel =
      ...
      (await prisma.user.findUnique({
        where: { email: authUserDel.email!.toLowerCase() },
        select: {
          id: true,
          role: true,
          accountStatus: true,
    ...
    if (dbUserDel && !dbUserDelBySupabaseId) {
      console.warn('[identity] Prisma row matched by email, not supabaseUserId', {
        supabaseUserId: authUserDel.id,
        prismaUserId: dbUserDel.id,
      })
    }
    ...
    if (user.role === 'PARENT') {
      ...
      if (!student?.parentId) {
        return NextResponse.json({ error: 'No parent is linked to your account.' }, { status: 400 })
      }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const dbUserGetBySupabaseId = await prisma.user.findUnique({
    ...
    if (dbUserGet && !dbUserGetBySupabaseId) {
      console.warn('[identity] Prisma row matched by email, not supabaseUserId', {
        supabaseUserId: authUserGet.id,
        prismaUserId: dbUserGet.id,
      })
    }
