...
export async function GET(request: NextRequest) {
  try {
    ...
    // Staff-only. Without a role gate ANY accepted member of ANY school -
    // including a pupil (school_members role='student') - passed this check,
    // and the studentId branch below never confirmed the target belonged to
    // the caller's school, so any member could harvest any minor's name,
    // email, progress and predicted grade across every school (the route uses
    // the service-role client, which bypasses RLS). Gate to staff for both
    // branches; scope the target to the caller's school below.
    ...
    if (classId) {
      ...
      // Confirm the requested pupil actually belongs to the caller's school
      // before returning their PII - mirrors students/[studentId]/trends.
      if (!(await verifyStudentInSchool(studentId, member.school_id))) {
    ...
    const [profilesResult, progressResult, coursesResult, modulesResult] = await Promise.all([
      admin.from('profiles').select('id, email, full_name, year_group').in('id', targetStudentIds),
      admin
    ...
    const students: StudentInsight[] = targetStudentIds.map((sid) => {
      ...
      if (scoredEntries.length >= 6) {
        ...
        const recentAvg =
          recentHalf.reduce((a, p) => a + (p.quiz_score as number), 0) / recentHalf.length
        const earlierAvg =
          earlierHalf.reduce((a, p) => a + (p.quiz_score as number), 0) / earlierHalf.length
      ...
      const skillScoreMap = new Map<
        string,
        { scores: number[]; recentScores: number[]; olderScores: number[] }
      >()
      ...
      for (const [moduleId, mod] of moduleMap) {
        ...
        if (weakSkillIndices.includes(skillIndex) && recommendedLessons.length < 5) {
          const courseName =
            courseMap.get((mod as { course_id: string }).course_id) || 'Unknown Course'
          recommendedLessons.push({
            ...
            priority:
              weaknesses.find((w) => w.area === skillAreas[skillIndex])!.score < 40
                ? 'high'
                : 'medium',
          })
        }
      ...
      if (completedDates.length > 0) {
        ...
        const activityDates = new Set(completedDates.map((d) => d.toISOString().split('T')[0]))
      ...
      const engagement: EngagementMetrics = {
        avg_session_duration_minutes:
          completedModules > 0 ? Math.round(totalTimeSeconds / completedModules / 60) : 0,
