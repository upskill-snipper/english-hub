...
export async function GET(request: NextRequest) {
  try {
    ...
    type ClassRow = { id: string; name: string; teacher_id: string | null; year_group: string | null; is_active: boolean };
    ...
    const yearGroupMap = new Map<
      string,
      { studentIds: string[]; atRiskCount: number }
    >();
    ...
    for (const [yg, data] of yearGroupMap) {
      ...
      const averageProgress =
        ygScores.length > 0
          ? Math.round(ygScores.reduce((a, b) => a + b, 0) / ygScores.length)
          : 0;
    ...
    if (topClasses.length === 0) {
      topClasses.push(
        { classId: "mock-1", className: "10A English", teacherName: "J. Smith", studentCount: 28, averageScore: 74 },
        { classId: "mock-2", className: "11B English", teacherName: "M. Jones", studentCount: 25, averageScore: 68 },
        { classId: "mock-3", className: "9C English",  teacherName: "R. Patel", studentCount: 30, averageScore: 61 }
      );
    }
    ...
    const hasRealActivity = weeklyActivity.some((w) => w.activeStudents > 0 || w.assignmentsSubmitted > 0);
    if (!hasRealActivity) {
