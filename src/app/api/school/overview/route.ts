...
export async function GET(request: NextRequest) {
  try {
    ...
    const [schoolResult, membersResult, classesResult] = await Promise.all([
      ...
      admin.from('classes').select('id').eq('school_id', schoolId).eq('is_active', true),
    ])
    ...
    if (studentIds.length > 0) {
      ...
      activeThisWeek = new Set((activeRows ?? []).map((r: { user_id: string }) => r.user_id)).size
    }
    ...
    if (classIds.length > 0) {
      ...
      try {
        assignmentsThisWeek = await prisma.assignment.count({
          where: { classId: { in: classIds }, createdAt: { gte: oneWeekAgo } },
        })
      } catch (err) {
        console.error('[api/school/overview] assignment count failed', err)
