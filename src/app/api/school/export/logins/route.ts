...
export async function GET(request: NextRequest) {
  try {
    ...
    const jobSchoolId =
      (job as { schoolId?: string; school_id?: string }).schoolId ??
      (job as { schoolId?: string; school_id?: string }).school_id

    if (jobSchoolId !== member.school_id) {
      return NextResponse.json(
        { error: 'Forbidden: job does not belong to your school' },
        { status: 403 },
      )
    }
    ...
    for (const u of users) {
      ...
      const row = [fullName, u.email, u.temporaryPassword, u.role, yearGroup, loginUrl].map(
        escapeCsvField,
      )
