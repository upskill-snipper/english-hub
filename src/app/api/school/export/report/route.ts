...
export async function GET(request: NextRequest) {
  try {
    ...
    // 5. Fetch all students in this school via the canonical helper (union
    //    of school_members role='student' and school_students so join-code
    //    pupils appear in the report too).
    let studentRows
    try {
      studentRows = await getSchoolStudents(admin, member.school_id as string)
    } catch (studentsError) {
    ...
    let classMap = new Map<string, { className: string; teacherName: string }>()
    ...
    let progressMap = new Map<string, { completed: number; scores: number[] }>()
