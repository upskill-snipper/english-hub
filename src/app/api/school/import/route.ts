  ...
  try {
    ...
    if (!isCSV) {
      return NextResponse.json(
        {
          error:
            'Only CSV files are supported. Please use the template to create your import file.',
        },
        { status: 422 },
      )
    }
    ...
    if (rows.length === 0) {
      return NextResponse.json({ error: 'The file is empty or has no data rows' }, { status: 422 })
    }
