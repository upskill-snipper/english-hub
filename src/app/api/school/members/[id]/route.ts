...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const { data: member, error: fetchError } = await admin
      .from("school_members")
      .select("id, user_id, role, full_name, email, department, invite_status, last_active_at, created_at, school_id")
      .eq("id", memberId)
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!actor) {
      return NextResponse.json({ error: "Forbidden: only school admins can update members" }, { status: 403 })
    }
