  ...
  try {
    ...
    if (!rl.success || !rlIp.success) {
      ...
      return NextResponse.json(
        {
          error:
            'Rate limit exceeded. Maximum 5 bulk commits per hour per school.',
        },
        {
          status: 429,
          headers: {
    ...
    if (body.rows.length > 500) {
      return NextResponse.json(
        { error: 'Maximum 500 rows per commit.' },
        { status: 422 },
      );
    }
      ...
      return NextResponse.json(
        {
          ...
          errors:
            (existing.errorsJson as unknown as ReadonlyArray<ParseError> | null) ?? [],
          outcomes: [],
          idempotent: true,
        },
        { status: 200 },
    ...
    const emails = Array.from(
      new Set(normalisedRows.map((r) => r.email.toLowerCase())),
    );
    ...
    try {
      ...
      return NextResponse.json(
        {
          error:
            'Commit failed. No accounts were created. Please review errors and retry.',
        },
        { status: 500 },
      );
    }
    ...
    for (const row of normalisedRows) {
      try {
        const email = row.email.toLowerCase();
        const authId =
          createdAuthIds.find((a) => a.email === email)?.authId ??
          existingByEmail.get(email)?.id;
        if (!authId) continue;
        ...
        console.warn(
          'bulk-upload.commit: school_members upsert failed',
          row.email,
          e,
        );
      }
