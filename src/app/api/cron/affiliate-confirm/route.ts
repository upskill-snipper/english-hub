...
export async function GET(request: NextRequest) {
  ...
  for (const referral of pendingReferrals) {
    try {
      ...
      console.error(`Failed to process referral ${referral.id}:`, err)
      // Continue processing other referrals
    }
  }

  console.log(`Affiliate commission confirmation: ${confirmed} confirmed, ${voided} voided out of ${pendingReferrals.length} processed`)

  return NextResponse.json({ confirmed, voided, total: pendingReferrals.length })
}
