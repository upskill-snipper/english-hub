      ...
      <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
        {!notes ? (
          ...
          <div className="space-y-6 print:space-y-3">
            {/* Provenance. This block is deliberately NOT print:hidden.
                It used to be, so the PDF a student downloaded carried no
                indication that the notes came from a fixed template rather
                than from the model - and the template footed itself
                "Generated by The English Hub AI Toolkit". The notes body now
                carries its own provenance footer from the API as well, so
                the claim survives printing, saving and copy-paste. */}
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                {aiGenerated
                  ...
                  : 'Assembled from a fixed template -- not AI-generated'}{' '}
                -- Grade {targetGrade} -- {topic}
              </span>
            </div>

            {/* Consistent AI-generated disclosure when the output was
                AI-generated (policy-matched, links to /legal/ai-governance). */}
            {aiGenerated && <AiGeneratedNotice variant="panel" />}
