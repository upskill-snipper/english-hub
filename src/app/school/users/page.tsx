      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 z-40 mt-1 w-44 rounded-lg border border-border bg-popover py-1 shadow-lg ring-1 ring-foreground/10">
            ...
            <button
              className="flex w-full items-center gap-2.5 px-3 py-1.5 text-sm text-foreground hover:bg-accent"
              onClick={() => { onResetPassword(user.id); setOpen(false) }}
            >
...
export default function UserManagementPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => { if (!cancelled) fetchUsers() }, 0)
    // Simulate loading for mock display
    const loadTimer = setTimeout(() => { if (!cancelled) setLoading(false) }, 800)
    return () => {
      cancelled = true
      clearTimeout(timer)
  ...
  useEffect(() => { setPage(1) }, [activeTab, search, roleFilter, yearFilter, statusFilter])
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Manage teachers, students, and admins across your school.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {PERMISSION_CARDS.map((card) => {
            ...
            return (
              <Card
                key={card.role}
                className={cn("border bg-card/50", card.borderClass)}
              >
                <CardContent className="flex items-start gap-3 pt-4 pb-4">
                  ...
                  <div className="min-w-0">
                    <p className={cn("text-sm font-semibold", card.colorClass)}>
                      {card.label}
                    </p>
