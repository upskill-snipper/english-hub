...
function buildDashboardState(): DashboardState {
  ...
  recentQuizzes.slice(0, 5).forEach((q, index) => {
    activities.push({
      ...
      type: 'quiz',
      title: q.mode ? `Quiz: ${q.mode}` : 'Quiz completed',
      meta:
        q.percentage != null
          ? `${q.percentage}%${q.grade ? ` · Grade ${q.grade}` : ''}`
          : 'Completed',
      date: q.date ?? null,
    })
  })
  ...
  games.slice(0, 5).forEach((g, index) => {
    activities.push({
      ...
      type: 'game',
      title: g.game ? `Game: ${g.game}` : 'Played a game',
      meta: g.score != null ? `Score ${g.score}` : 'Completed',
      date: g.date ?? null,
    })
  })

  poems.slice(-5).reverse().forEach((slug, index) => {
    const friendly = slug
      .split('-')
    ...
    activities.push({
      id: `poem-${index}-${slug}`,
      type: 'poem',
      title: `Studied "${friendly}"`,
      meta: 'Poetry revision',
      date: null,
    })
  })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {mounted
              ? `A snapshot of ${firstName}'s learning on The English Hub.`
              : 'Loading dashboard...'}
          </p>
        </div>
      ...
      <ChildProgressCard
        ...
        workingAtGrade={state?.averageQuizScore != null ? Math.max(1, Math.min(9, Math.round(state.averageQuizScore / 11))) : undefined}
        predictedGrade={state?.averageQuizScore != null ? Math.max(1, Math.min(9, Math.round(state.averageQuizScore / 11) + 1)) : undefined}
        targetGrade={7}
      ...
      {/* This week snapshot */}
      <div>
        <h3 className="mb-3 text-lg font-semibold text-foreground">This Week</h3>
            ...
            <CardContent className="flex items-center gap-3 p-4">
              ...
              <div>
                <p className="text-2xl font-bold text-foreground">
                  {state ? `${Math.floor(state.totalMinutesThisWeek / 60)}h ${state.totalMinutesThisWeek % 60}m` : '—'}
                </p>
          ...
          <Card>
            <CardContent className="flex items-center gap-3 p-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Poems studied</p>
              </div>
            </CardContent>
          </Card>
      ...
      <WeeklyActivityChart
        data={state?.weekly ?? []}
        title="Last 8 weeks"
        description="Estimated time spent learning each week"
      />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Grades summary</CardTitle>
            <CardDescription>Based on recent quiz results</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">Poems studied</span>
              </div>
              <span className="text-sm font-bold text-foreground">
                {state?.poemCount ?? 0}
              </span>
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>Last few things {firstName} worked on</CardDescription>
          </CardHeader>
          <CardContent>
            {state && state.recentActivity.length > 0 ? (
              ...
              <p className="rounded-lg border border-dashed border-border p-6 text-center text-sm text-muted-foreground">
                {mounted
                  ? 'No activity yet. Come back once your child has completed some lessons.'
                  : 'Loading activity...'}
              </p>
            )}
          </CardContent>
        ...
        >
          <span className="flex items-center gap-3">
            ...
            <span className="text-left">
              ...
              <span className="block text-xs text-muted-foreground">
                Preview what lands in your inbox
              </span>
            </span>
          </span>
          <ArrowRight className="h-4 w-4 text-muted-foreground" />
