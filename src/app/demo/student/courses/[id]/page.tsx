...
const COURSES: Record<string, CourseData> = {
  "igcse-lang-p1": {
    ...
    modules: [
      ...
      { id: "m13", title: "Commenting on Structure", completed: false, lessonContent: "Structure refers to how a text is organised and how ideas are sequenced. Examiners want to see that you can comment on structural choices.\n\nStructural features to look for:\n- Opening and ending: How does the text begin and conclude?\n- Shifts in focus: Does the writer move from one idea to another?\n- Paragraph length: Short paragraphs create pace; long ones develop ideas\n- Chronological vs non-chronological order\n- Use of flashback or foreshadowing\n- Circular structure: ending mirrors the beginning\n\nWhen commenting on structure, always explain WHY the writer has made these choices. What effect does the structure have on the reader?", quiz: [
...
function getGenericCourse(id: string): CourseData {
  const lookup: Record<string, { title: string; description: string; progress: number; moduleCount: number }> = {
    "igcse-lang-p2": { title: "IGCSE English Language Paper 2", description: "Extended writing, descriptive and narrative composition techniques.", progress: 45, moduleCount: 12 },
    "igcse-lit-inspector": { title: "IGCSE Literature -- Inspector Calls", description: "Character analysis, themes, and essay writing for An Inspector Calls.", progress: 80, moduleCount: 10 },
    "igcse-lit-poetry": { title: "IGCSE Literature -- Poetry", description: "Poetry analysis, comparison essays, and unseen poetry techniques.", progress: 30, moduleCount: 8 },
          ...
          <div className="space-y-2">
            {course.modules.map((mod, idx) => (
              <button
                ...
                className="w-full group flex items-center gap-3 rounded-lg border border-white/5 bg-white/[0.02] px-4 py-3.5 text-left transition-all hover:border-white/10 hover:bg-white/[0.04]"
              >
                {mod.completed ? (
          ...
          <>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 rounded-lg bg-blue-600 hover:bg-blue-500 px-4 py-3 text-sm font-medium text-white transition-colors"
              >
