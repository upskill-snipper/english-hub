...
const STATUS_COLORS: Record<StudentStatus, string> = {
  'On Track': 'bg-primary/10 text-primary border-primary/30',
  'Needs Support': 'bg-amber-500/15 text-amber-700 dark:text-amber-300 border-amber-500/30',
  'At Risk': 'bg-red-500/15 text-red-700 dark:text-red-300 border-red-500/30',
  Excelling: 'bg-yellow-400/15 text-amber-700 dark:text-amber-300 border-yellow-400/30',
}

const YEAR_COLORS: Record<number, string> = {
  7: 'bg-primary/15 text-primary border-primary/30',
  8: 'bg-indigo-500/15 text-indigo-700 dark:text-indigo-300 border-indigo-500/30',
  9: 'bg-primary/10 text-primary border-primary/30',
  10: 'bg-primary/10 text-primary border-primary/30',
  11: 'bg-amber-500/15 text-amber-700 dark:text-amber-300 border-amber-500/30',
  12: 'bg-rose-500/15 text-rose-700 dark:text-rose-300 border-rose-500/30',
  13: 'bg-pink-500/15 text-pink-700 dark:text-pink-300 border-pink-500/30',
}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {paginated.map((student) => {
                  const avatarColor =
                    INITIALS_COLORS[seededHash(student.id) % INITIALS_COLORS.length]
                  return (
                    <tr
                      key={student.id}
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold ${YEAR_COLORS[student.yearGroup] ?? 'bg-muted text-muted-foreground border-border'}`}
                        >
                          Y{student.yearGroup}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 hidden sm:table-cell">
                        <span
                          className={`tabular-nums font-medium ${gcseGradeColor(student.workingAtGrade)}`}
                        >
                      ...
                      <td className="px-4 py-3 hidden md:table-cell">
                        <span
                          className={`tabular-nums font-medium ${predictedGradeColor(student.predictedGrade, student.workingAtGrade)}`}
                        >
                      ...
                      <td className="px-4 py-3 hidden xl:table-cell">
                        <ReadingAgeInline
                          ...
                          yearGroup={student.yearGroupStr}
                        />
                      </td>
                      <td className="px-4 py-3">
                        <span
