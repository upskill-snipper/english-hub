...
// This report prints the pupil's modules, mocks, essays and quiz history, so it
// needs the full roster. The roster is ~320 KB and was a module-scope import,
// which put all of it into this route's First Load JS (and into ~19 other demo
// routes that never render any of it). It now loads as a separate async chunk.
...
function statusDot(status: string): string {
  switch (status) {
    case 'excelling':
    case 'Excelling':
      return 'bg-primary print:bg-teal-800'
    case 'on-track':
    case 'On Track':
      return 'bg-blue-500 print:bg-teal-800'
    case 'needs-support':
    case 'Needs Support':
      return 'bg-amber-500 print:bg-amber-600'
    case 'at-risk':
    case 'At Risk':
      return 'bg-red-500 print:bg-red-600'
    default:
...
function getScoreTrend(scores: number[]): { label: string; description: string } {
  if (scores.length < 2)
    return {
      ...
      description: 'Not enough assessment data to determine a trend.',
    }
  ...
  if (diff > 5)
    return {
      label: 'Improving',
      description: `Scores have improved by an average of ${Math.round(diff)} percentage points across recent assessments, indicating strong upward momentum.`,
    }
  if (diff < -5)
    return {
      label: 'Declining',
      description: `Scores have dropped by an average of ${Math.round(Math.abs(diff))} percentage points across recent assessments. Intervention may be required.`,
    }
  return {
    label: 'Stable',
    description:
      'Scores have remained consistent across recent assessments. Encourage continued effort and targeted practice to push for improvement.',
  }
}
...
export default function StudentReportPage() {
  ...
  // the report would flash "student not found" on every cold load.
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <p className="mt-2 text-muted-foreground">No student exists with this identifier.</p>
          <Link
  ...
  const completionPct = Math.round((student.assignmentsCompleted / student.assignmentsTotal) * 100)
  const avgQuizPct =
      ...
      <div className="print-container px-4 py-8 space-y-6">
            ...
            <button
              onClick={() => {
                toast.success('Secure link generated', {
                  description:
                    "In production, this sends a secure, time-limited link to the student's parents/carers via email. They can view the report without needing an account.",
                })
              }}
        ...
        <div className="report-header bg-card border border-border rounded-xl p-8 print:rounded-none">
          ...
          <div className="mt-6 grid grid-cols-2 gap-4 text-sm">
            <div>
              <span className="text-muted-foreground print:text-neutral-500">Student:</span>{' '}
              <span className="text-foreground print:text-black font-medium">{student.name}</span>
            </div>
            <div>
        ...
        <div className="report-card bg-card border border-border rounded-xl p-6 print:rounded-none">
          ...
          />
          {student.readingAge && (
            <div className="mt-3 text-sm text-muted-foreground print:text-neutral-500">
              Reading Age:{' '}
              <span className="font-semibold text-muted-foreground print:text-black">
                {formatReadingAge(student.readingAge)}
              </span>
            </div>
          )}
        </div>
        ...
        <div className="report-card bg-card border border-border rounded-xl p-6 print:rounded-none">
          ...
          <div className="flex items-center gap-8">
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground print:text-neutral-500 mt-1">
                Working At Grade
              </div>
            </div>
            <div className="flex-1 grid grid-cols-4 gap-3 text-sm">
              <div className="bg-muted print:bg-neutral-100 rounded-lg p-3 text-center">
                <div
              ...
              <div className="bg-muted print:bg-neutral-100 rounded-lg p-3 text-center">
                <div className="flex items-center justify-center gap-1.5">
                  <span
                    className={`inline-block size-2.5 rounded-full ${statusDot(student.status)}`}
                  />
                ...
                <tbody>
                  {allAssessments.map((a, i) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5 text-center">
                        <span
                          className={`inline-block text-xs px-2 py-0.5 rounded-full font-medium ${
                            a.type === 'Mock Exam'
                              ? 'bg-primary/15 text-primary print:bg-teal-100 print:text-violet-700'
                              : a.type === 'Essay'
                                ? 'bg-primary/15 text-primary print:bg-teal-100 print:text-violet-600'
                                : 'bg-primary/10 text-primary/80 print:bg-teal-50 print:text-violet-600'
                          }`}
                        >
                          {a.type}
                        </span>
                      ...
                      <td className="px-3 py-2.5 text-center text-muted-foreground print:text-black font-medium">
          ...
          <div className="flex items-center gap-3 mb-3">
            <span
              className={`inline-flex items-center gap-1.5 text-sm font-semibold px-3 py-1 rounded-full ${
                scoreTrend.label === 'Improving'
                  ? 'bg-primary/10 text-primary print:bg-teal-50 print:text-emerald-700'
                  : scoreTrend.label === 'Declining'
                    ? 'bg-red-500/15 text-red-700 dark:text-red-300 print:bg-red-100 print:text-red-700'
                    : 'bg-blue-500/15 text-primary print:bg-blue-100 print:text-blue-700'
              }`}
            >
              {scoreTrend.label === 'Improving' && (
                <svg
        ...
        <div className="report-card bg-card border border-border rounded-xl p-6 print:rounded-none">
          ...
          <div className="space-y-3">
            {student.modules.map((mod) => {
              const pct =
                mod.progress ??
                (mod.status === 'Complete' ? 100 : mod.status === 'In Progress' ? 50 : 0)
              return (
                <div key={mod.name} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 bg-muted print:bg-neutral-200 rounded-full h-3 overflow-hidden">
                    <div
                    ...
                    className={`text-xs px-2 py-0.5 rounded-full font-medium w-24 text-center ${
                      mod.status === 'Complete'
                        ...
                        : mod.status === 'In Progress'
                          ? 'bg-blue-500/15 text-primary print:bg-blue-100 print:text-blue-700'
                          : 'bg-muted text-muted-foreground print:bg-neutral-200 print:text-neutral-500'
                    }`}
                  >
        ...
        {/* Strengths */}
        <div className="report-card bg-card border border-border rounded-xl p-6 print:rounded-none">
          <h2 className="text-lg font-semibold text-foreground print:text-black mb-3">Strengths</h2>
          <p className="text-sm text-muted-foreground print:text-black leading-relaxed">
              ...
              (() => {
                ...
                return (
                  <>
                    ...
                    <strong className="text-foreground print:text-black">{firstName}</strong>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {student.strengths.map((s) => {
              ...
              return (
                <span
                  key={sName}
                  className="text-xs px-2.5 py-1 rounded-full bg-primary/10 text-primary border border-primary/20 print:bg-teal-50 print:text-emerald-700 print:border-emerald-300"
                >
                  {sName}
                  {sScore !== null && <>: {sScore}%</>}
                </span>
          ...
          <p className="text-sm text-muted-foreground print:text-black leading-relaxed">
            {student.weaknesses.length > 0 ? (
              <>
                ...
                {student.weaknesses
                  .map((w) => (typeof w === 'string' ? w : w.name).toLowerCase())
                  .join(', ')}
                ...
                {(() => {
                  ...
                  return (
                    <>
                      <strong className="text-foreground print:text-black">{lastName}</strong>
              ...
              <>
                {student.name} is performing well across all assessed areas. Continue to challenge
                with extension work.
              </>
            )}
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            {student.weaknesses.map((w) => {
              ...
              return (
                <span
                  key={wName}
                  className="text-xs px-2.5 py-1 rounded-full bg-amber-500/10 text-amber-700 dark:text-amber-300 border border-amber-500/30 print:bg-amber-50 print:text-amber-700 print:border-amber-300"
                >
                  {wName}
                  {wScore !== null && <>: {wScore}%</>}
                </span>
        ...
        <div className="report-card bg-card border border-border rounded-xl p-6 print:rounded-none">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div className="bg-muted print:bg-neutral-100 rounded-lg p-3 text-center">
              <div className="text-xl font-bold text-foreground print:text-black">
        ...
        <div className="report-card bg-card border border-border rounded-xl p-6 print:rounded-none">
          ...
          <div className="bg-muted print:bg-neutral-50 rounded-lg p-4 border border-border print:border-neutral-300">
            <p className="text-sm text-muted-foreground print:text-black leading-relaxed italic">
        ...
        {/* Previous Reports */}
        <div className="no-print report-card bg-card border border-border rounded-xl p-6">
          ...
          <div className="space-y-2">
            {[
              ...
              <div
                key={report.date}
                className="flex items-center justify-between bg-muted rounded-lg px-4 py-3 text-sm"
              >
                <div className="flex items-center gap-3">
                  <svg
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() =>
                      toast.info('In the full product, this would open the archived report PDF.')
                    }
        ...
        <div className="no-print report-card bg-card border border-border rounded-xl p-6">
          ...
          <p className="text-xs text-muted-foreground mb-3">
            Add additional notes for this report. In the full product, notes are saved and included
            in the exported PDF.
          </p>
          <textarea
            value={teacherNotes}
            ...
            className="w-full bg-muted border border-border rounded-lg p-4 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-ring resize-y min-h-[100px]"
            rows={4}
          />
              ...
              onClick={() => {
                if (teacherNotes.trim()) {
                  toast.success('Notes saved', {
                    description:
                      'In the full product, these notes would be saved and included in future report exports.',
                  })
                } else {
        ...
        <div className="text-center text-xs text-muted-foreground print:text-neutral-500 pt-4 border-t border-border print:border-neutral-300">
          <p>The English Hub - Student Progress Report - Confidential</p>
          <p className="mt-1">
            Generated on{' '}
            {new Date().toLocaleDateString('en-GB', {
              day: 'numeric',
