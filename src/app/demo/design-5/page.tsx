...
const mockData = {
  ...
  courses: [
    { name: 'Paper 1: Creative Reading & Writing', emoji: '📖', progress: 72, color: 'from-emerald-500 to-cyan-500' },
    { name: "Paper 2: Writers' Viewpoints", emoji: '✍️', progress: 45, color: 'from-orange-500 to-pink-500' },
    ...
    { name: 'Unseen Poetry', emoji: '🔮', progress: 60, color: 'from-blue-500 to-indigo-600' },
...
export default function DopamineDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#0f0720] text-white relative overflow-hidden">
      {/* Gradient blob */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[900px] h-[600px] bg-gradient-to-b from-purple-600/20 via-pink-600/10 to-transparent rounded-full blur-3xl pointer-events-none" />

      <div className="relative z-10 max-w-5xl mx-auto px-4 py-8">
        {/* Header */}
        <div className="flex items-center justify-between mb-8">
          <div>
            <p className="text-purple-300 text-sm font-medium tracking-wide uppercase">English Hub</p>
            <h1 className="text-2xl font-bold mt-1">Hey, {mockData.user.name}! 👋</h1>
          </div>
          <div className="flex items-center gap-3">
            <div className="bg-white/10 backdrop-blur rounded-full px-4 py-2 flex items-center gap-2 text-sm font-semibold">
        ...
        {/* Hero Banner */}
        <div className="bg-gradient-to-r from-purple-900/60 via-pink-900/40 to-purple-900/60 backdrop-blur rounded-[20px] p-8 mb-6 border border-white/5 text-center">
          <div className="text-5xl mb-3 animate-bounce inline-block">🔥</div>
          <h2 className="text-4xl font-extrabold mb-2">
            You&apos;re on fire!
          </h2>
          <p className="text-lg text-purple-200 mb-4">You&apos;ve been studying for</p>
          ...
          <p className="text-purple-300 text-sm mt-3">Keep it up! Your longest streak was 23 days.</p>
        </div>

        {/* Stats Row */}
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-gradient-to-br from-emerald-500 to-cyan-500 rounded-[20px] p-6">
            <span className="text-3xl">⭐</span>
            <p className="text-sm font-medium text-white/80 mt-2">Total Score</p>
            <p className="text-3xl font-extrabold mt-1">87%</p>
            <p className="text-xs text-white/70 mt-1">Avg. across all quizzes</p>
          </div>
          <div className="bg-gradient-to-br from-orange-500 to-pink-500 rounded-[20px] p-6">
            <span className="text-3xl">🎯</span>
            <p className="text-sm font-medium text-white/80 mt-2">Weekly Goal</p>
          ...
          <div className="bg-gradient-to-br from-violet-500 to-purple-600 rounded-[20px] p-6">
            <span className="text-3xl">📚</span>
            <p className="text-sm font-medium text-white/80 mt-2">Courses Active</p>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-gradient-to-br from-rose-500 to-red-600 rounded-[20px] p-6 flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-bold uppercase tracking-widest text-white/70">Daily Challenge</span>
              </div>
              <h3 className="text-2xl font-extrabold mb-2">{mockData.dailyChallenge.title}</h3>
              <p className="text-white/80 text-sm">{mockData.dailyChallenge.description}</p>
            </div>
            <div className="mt-5">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-semibold text-white/70">⏰ {mockData.dailyChallenge.timeLeft} left</span>
          ...
          {/* Weekly XP Chart */}
          <div className="bg-white/5 backdrop-blur border border-white/10 rounded-[20px] p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-lg">This Week</h3>
              <span className="text-sm text-purple-300 font-semibold">{totalWeeklyXp} XP</span>
            </div>
            <div className="flex items-end gap-2 h-32">
              {mockData.weeklyXp.map((xp, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  <span className="text-[10px] text-purple-300 font-medium">{mockData.weekDays[i]}</span>
                </div>
              ))}
            </div>
            <p className="text-xs text-purple-400 mt-3 text-center">🏅 Best day: Friday — 600 XP</p>
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="text-sm text-purple-300 hover:text-white transition-colors font-medium">View All →</button>
          </div>
          ...
          <div className="bg-white/5 backdrop-blur border border-white/10 rounded-[20px] p-6">
            ...
            <div className="flex flex-wrap gap-3">
              {mockData.achievements.map((badge, i) => (
                ...
                >
                  <div className="w-14 h-14 rounded-full bg-gradient-to-br from-purple-500/30 to-pink-500/30 border border-white/10 flex items-center justify-center text-2xl hover:scale-110 hover:border-white/30 transition-all duration-200 cursor-pointer">
                    {badge.emoji}
                  </div>
                  <span className="text-[10px] text-purple-300 mt-1 font-medium">{badge.label}</span>
                </div>
              ))}
            </div>
            <button className="mt-4 text-sm text-purple-400 hover:text-white transition-colors font-medium">
              See all 24 badges →
            </button>
          </div>
              ...
              {mockData.recentActivity.map((item, i) => (
                <div key={i} className="flex items-center justify-between bg-white/5 rounded-xl px-4 py-3">
                  <div>
                    ...
                    <p className="text-xs text-purple-400 mt-0.5">{item.time}</p>
                  </div>
        ...
        <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 rounded-[20px] p-6 mb-8">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-white/70">820 XP to Level {mockData.user.level + 1}</p>
              </div>
            </div>
            <span className="text-3xl">🚀</span>
          </div>
        ...
        {/* Footer */}
        <div className="text-center text-purple-500 text-xs pb-4">
