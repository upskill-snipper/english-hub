...
function InitialsAvatar({ name, color = 'bg-cream-100' }: { name: string; color?: string }) {
  ...
  return (
    <div
      className={`w-9 h-9 rounded-full ${color} flex items-center justify-center text-xs font-semibold text-ink-900 shrink-0`}
    >
      {initials}
    </div>
  )
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="rounded-xl border border-teal-800/10 bg-gradient-to-br from-teal-800/10 via-teal-800/5 to-transparent p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-teal-700/70 font-medium uppercase tracking-wider">
                  {t('teacher_page.demo.stat.classes')}
                </span>
              </div>
              <p className="text-3xl font-bold text-ink-900">{TEACHER_DEMO_CLASSES.length}</p>
              <p className="text-xs text-ink-500 mt-1">{t('teacher_page.demo.stat.classes_sub')}</p>
            </div>
            ...
            <div className="rounded-xl border border-clay-500/15 bg-gradient-to-br from-clay-500/10 via-purple-500/5 to-transparent p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-clay-600/70 font-medium uppercase tracking-wider">
                ...
                <div className="space-y-4">
                  {TEACHER_DEMO_LESSONS.map((lesson, idx) => (
                    <div key={lesson.id} className="flex items-start gap-4 relative">
                      {/* Timeline dot */}
                      <div
                        className={`w-[9px] h-[9px] rounded-full mt-1.5 shrink-0 relative z-10 ${idx === 0 ? 'bg-teal-600 ring-2 ring-emerald-400/20' : 'bg-white/20'}`}
                      />
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <Badge
                        variant="outline"
                        className="text-[10px] border-teal-800/20 text-teal-700/70"
                      >
                        {assignment.dueDate}
                      </Badge>
                    </div>
          ...
          <div className="rounded-xl border border-ink-200 bg-white p-5">
            ...
            <div className="flex items-center gap-4 mb-5">
              <div className="flex-1">
                ...
                <div className="h-2 rounded-full bg-cream-100 overflow-hidden">
                  <div
                    className="h-2 rounded-full bg-gradient-to-r from-clay-500 to-purple-400 transition-all"
            ...
            {pendingMarking.length > 0 ? (
              <div className="space-y-2">
                <p className="text-xs text-ink-500 font-medium uppercase tracking-wider mb-3">
              ...
              <div className="text-center py-4">
                ...
                <p className="text-sm text-ink-500">{t('teacher_page.demo.all_caught_up')}</p>
              </div>
            )}
          </div>
        </section>
