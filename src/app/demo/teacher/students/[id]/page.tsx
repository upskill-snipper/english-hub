...
function statusBadge(status: string) {
  const styles: Record<string, string> = {
    excelling: 'bg-green-500/10 text-green-600 dark:text-green-400 border-green-500/20',
    'on-track': 'bg-primary/10 text-primary border-primary/20',
    'needs-support': 'bg-amber-500/10 text-clay-600 dark:text-clay-400 border-amber-500/20',
    'at-risk': 'bg-red-500/10 text-red-600 dark:text-red-400 border-red-500/20',
  }
  return styles[status] || 'bg-muted/40 text-muted-foreground border-neutral-500/20'
}
...
export default function TeacherStudentProfilePage() {
  ...
  const classAvgScore =
    classmates.length > 0
      ? Math.round(
          classmates.reduce((sum: number, s: DemoStudent) => sum + s.averageScore, 0) /
            classmates.length,
        )
      : 0
  const classAvgProgress =
    classmates.length > 0
      ? Math.round(
          classmates.reduce((sum: number, s: DemoStudent) => sum + s.overallProgress, 0) /
            classmates.length,
        )
      : 0
  const classAvgAssignments =
    classmates.length > 0
      ? Math.round(
          classmates.reduce((sum: number, s: DemoStudent) => sum + s.assignmentsCompleted, 0) /
            classmates.length,
        )
      : 0
    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="flex items-center justify-between mb-6">
          <Link
            href="/demo/teacher/classes"
            className="inline-flex items-center gap-1.5 text-[11px] uppercase tracking-[0.2em] text-muted-foreground hover:text-muted-foreground transition-colors"
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
          ...
          <Card
            className={`border ${predictedGradeBg(student.predictedGrade, student.workingAtGrade)}`}
          >
            <CardContent className="p-5">
              <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground mb-1">
                Predicted Grade
          ...
          <CardContent>
            <div className="space-y-3">
              {getPersonalisedRecommendations(
                ...
                >
                  <span className="mt-0.5 h-5 w-5 shrink-0 rounded-full bg-amber-500/15 text-clay-600 dark:text-clay-400 flex items-center justify-center text-[10px] font-bold">
                    {i + 1}
                  </span>
          ...
          <CardContent>
            <div className="mb-4">
              <GradeProgressCard
                ...
                trend={
                  student.recentScores.length >= 2 &&
                  student.recentScores[student.recentScores.length - 1] > student.recentScores[0]
                    ? 'up'
                    : student.recentScores.length >= 2 &&
            ...
            <GradeRecommendations
              currentGrade={percentageToGCSEGrade(student.averageScore)}
              weakAreas={(student.weaknesses || []).map((w) =>
              ...
              {(() => {
                ...
                const topicSkills: {
                ...
                }[] = [
                  {
                    ...
                    status: (() => {
                      const mod = student.moduleProgress.find(
                        (m) =>
                          m.module.toLowerCase().includes('reading') ||
                          m.module.toLowerCase().includes('comprehension'),
                      )
                  ...
                  },
                  {
                    name: 'Essay Writing',
                    status: (() => {
                      const essayScores = student.essaySubmissions
                        .filter((e) => e.score > 0)
                        .map((e) => e.score)
                  ...
                  },
                  {
                    ...
                    status: (() => {
                      const mod = student.moduleProgress.find((m) =>
                        m.module.toLowerCase().includes('creative'),
                      )
                      if (mod && mod.score >= 70) return 'mastered'
                      if (mod && mod.score >= 50) return 'developing'
                      const hasCreativeEssay = student.essaySubmissions.some((e) =>
                        e.title.toLowerCase().includes('creative'),
                      )
                      if (hasCreativeEssay) {
                        const score =
                          student.essaySubmissions.find((e) =>
                            e.title.toLowerCase().includes('creative'),
                          )?.score ?? 0
                    ...
                    status: (() => {
                      const hasStrength = student.strengths.some((s) => {
                        ...
                        return (
                          ...
                          name.toLowerCase().includes('punctuation') ||
                          name.toLowerCase().includes('spag')
                        )
                      })
                  ...
                  },
                  {
                    ...
                    status: (() => {
                      const mod = student.moduleProgress.find(
                        (m) =>
                          m.module.toLowerCase().includes('inspector') ||
                          m.module.toLowerCase().includes('macbeth') ||
                          m.module.toLowerCase().includes('literature'),
                      )
                      if (mod && mod.score >= 70) return 'mastered'
                      if (mod && mod.score >= 50) return 'developing'
                  ...
                  },
                  {
                    name: 'Theme Analysis',
                    status: (() => {
                      const hasEssay = student.essaySubmissions.some((e) =>
                ...
                return topicSkills.map((skill, i) => (
                  ...
                  >
                    <p className="text-xs font-medium text-muted-foreground mb-1.5">{skill.name}</p>
                    <span
                      className={`inline-block text-[10px] font-medium uppercase tracking-wider px-2 py-0.5 rounded-full ${
                        skill.status === 'mastered'
                          ? 'bg-green-500/15 text-green-600 dark:text-green-400'
                          : skill.status === 'developing'
                            ? 'bg-amber-500/15 text-clay-600 dark:text-clay-400'
                            : 'bg-red-500/15 text-red-600 dark:text-red-400'
                      }`}
                    >
                      {skill.status === 'mastered'
                ...
                {(() => {
                  ...
                  return allAssessments.map((item, i) => {
                    ...
                    const prevPct =
                      i > 0
                        ? Math.round(
                            (allAssessments[i - 1].score / allAssessments[i - 1].maxScore) * 100,
                          )
                        : null
                    ...
                    return (
                      <div key={i} className="flex items-start gap-4 pl-1">
                        ...
                        <div className="flex-1 min-w-0 pb-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <Badge
                              ...
                              className={`text-[9px] ${
                                item.type === 'Essay'
                                  ...
                                  : item.type === 'Quiz'
                                    ? 'border-amber-500/20 text-clay-600 dark:text-clay-400'
                                    : 'border-clay-500/20 text-clay-600 dark:text-clay-400'
                              }`}
                            >
                              {item.type}
                            </Badge>
                            {change !== null && change !== 0 && (
                              <span
                                className={`text-[10px] flex items-center gap-0.5 ${change > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                              >
                                {change > 0 ? (
                        ...
                        <span
                          className={`text-sm tabular-nums font-medium shrink-0 ${scoreColor(pct)}`}
                        >
          ...
          <CardContent>
            <div className="space-y-3">
              {student.strengths.map((s, i) => {
                ...
                const relatedEssay = student.essaySubmissions.find(
                  (e) =>
                    e.title.toLowerCase().includes(name.toLowerCase().split(' ')[0]) &&
                    e.score >= 65,
                )
                const relatedQuiz = student.quizAttempts.find(
                  (q) =>
                    q.quiz.toLowerCase().includes(name.toLowerCase().split(' ')[0]) &&
                    q.score / q.maxScore >= 0.65,
                )
                return (
                  <div
                    key={i}
                    className="rounded-lg border border-green-500/10 bg-green-500/[0.03] p-3"
                  >
            ...
            <div className="space-y-3">
              {student.weaknesses.map((w, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="rounded-lg border border-red-500/10 bg-red-500/[0.03] p-3"
                  >
                    <div className="flex items-center justify-between mb-1">
            ...
            <div className="space-y-3">
              {(() => {
                ...
                if (essayScores.length >= 2) {
                  ...
                  if (prevTwo.length > 0) {
                    const recentAvg = Math.round(
                      lastTwo.reduce((a, b) => a + b, 0) / lastTwo.length,
                    )
                    const prevAvg = Math.round(prevTwo.reduce((a, b) => a + b, 0) / prevTwo.length)
                    if (recentAvg < prevAvg - 5) {
                      suggestions.push(
                        `Focus on essay paragraph structure - score dropped from ${prevAvg}% to ${recentAvg}% over last ${lastTwo.length} essays`,
                      )
                    }
                  }
                }
                ...
                if (weakQuizzes.length > 0) {
                  suggestions.push(
                    `Revise topics from failed quizzes: ${weakQuizzes
                      .map((q) => q.quiz)
                      .slice(0, 2)
                      .join(', ')}`,
                  )
                ...
                if (incompleteModules.length > 0) {
                  suggestions.push(
                    `Complete outstanding modules: ${incompleteModules
                      .map((m) => m.module)
                      .slice(0, 2)
                      .join(', ')} (under 50% progress)`,
                  )
                }
                // Check assignments completion rate
                if (student.assignmentsCompleted < student.assignmentsTotal * 0.8) {
                  suggestions.push(
                    `Catch up on ${student.assignmentsTotal - student.assignmentsCompleted} outstanding assignments to stay on track`,
                  )
                }
                ...
                if (mockScores.length > 0 && mockScores[mockScores.length - 1] < 60) {
                  suggestions.push(
                    `Book a revision session before next mock exam - last score was ${mockScores[mockScores.length - 1]}%`,
                  )
                }
                ...
                if (suggestions.length < 3) {
                  student.recommendations
                    ...
                    .forEach((r) => suggestions.push(r))
                }
                return suggestions.map((suggestion, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-3 rounded-lg border border-amber-500/10 bg-amber-500/[0.03] p-3"
                  >
                    <span className="shrink-0 mt-0.5 h-5 w-5 rounded-full bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-[10px] text-clay-600 dark:text-clay-400 tabular-nums font-medium">
                      {i + 1}
                    </span>
                ...
                <div className="space-y-2">
                  <div>
                    <div className="flex items-center justify-between text-xs mb-1">
                      <span className="text-primary">{student.name.split(' ')[0]}</span>
        ...
        <Card className="bg-card border-border/60 overflow-hidden mb-8">
          ...
          {student.modules.map((mod: any, i: number) => (
            <div
              key={i}
              className="grid grid-cols-1 sm:grid-cols-[1fr_160px_80px_80px] gap-1 sm:gap-4 px-5 py-3 border-b border-white/[0.03]"
            >
                  ...
                  className={`text-[10px] ${
                    (mod.status ?? '') === 'Complete'
                      ...
                      : (mod.status ?? '') === 'In Progress'
                        ? 'border-amber-500/20 text-clay-600 dark:text-clay-400'
                        : 'border-neutral-500/20 text-muted-foreground'
                  }`}
                >
                  {mod.status ?? 'In Progress'}
                </Badge>
        ...
        <Card className="bg-card border-border/60 overflow-hidden mb-8">
          ...
          {student.mockExams.map((exam: any, i: number) => (
            <div
              key={i}
              className="grid grid-cols-1 sm:grid-cols-[1fr_100px_80px_60px] gap-1 sm:gap-4 px-5 py-3 border-b border-white/[0.03]"
            >
              <p className="text-sm text-foreground">{exam.name}</p>
              <p className="text-sm text-muted-foreground">{exam.date}</p>
              <p
                className={`text-sm tabular-nums text-center ${gcseGradeColor(percentageToGCSEGrade(exam.score))}`}
              >
                {percentageToGCSEGradeLabel(exam.score)}
              </p>
              <p className="text-sm font-medium text-center text-muted-foreground">{exam.grade}</p>
            </div>
          ))}
        </Card>
          ...
          {student.essays.map((essay: any, i: number) => (
            <div key={i} className="px-5 py-4 border-b border-white/[0.03]">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mb-2">
                <div className="flex items-center gap-2">
                  <p className="text-sm text-foreground">{essay.title}</p>
                  {essay.teacherReviewed && (
                    <Badge
                      variant="outline"
                      className="text-[9px] border-green-500/20 text-green-600 dark:text-green-400"
                    >
                      Reviewed
                    </Badge>
                  )}
              ...
              <p className="text-xs text-muted-foreground italic">{essay.feedback}</p>
            </div>
          ))}
        </Card>
        ...
        <Card className="bg-card border-border/60 overflow-hidden mb-8">
          ...
          {student.quizAttempts.map((quiz: any, i: number) => {
            ...
            return (
              <div
                key={i}
                className="grid grid-cols-1 sm:grid-cols-[1fr_100px_100px] gap-1 sm:gap-4 px-5 py-3 border-b border-white/[0.03]"
              >
            ...
            <div className="space-y-4">
              {student.activityTimeline.map((item: any, i: number) => {
                const isPositive =
                  ...
                  item.action.startsWith('Achieved') ||
                  item.action.startsWith('Scored')
                const isNegative =
                  item.action === 'Absent' ||
                  item.action === 'Incomplete' ||
                  item.action.includes('late')
                return (
                  <div key={i} className="flex items-start gap-3">
                    <div className="flex flex-col items-center mt-1">
                      <div
                        className={`h-2 w-2 rounded-full ${isPositive ? 'bg-primary' : isNegative ? 'bg-red-500' : 'bg-primary'}`}
                      />
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-muted-foreground">
                        <span
                          className={`font-medium ${isPositive ? 'text-primary' : isNegative ? 'text-red-600 dark:text-red-400' : 'text-primary'}`}
                        >
                          {item.action}
                        </span>
                        {' -- '}
            ...
            <ul className="space-y-3">
              {student.recommendations.map((rec, i) => (
                <li key={i} className="flex items-start gap-3 text-sm text-muted-foreground">
                  <span className="shrink-0 mt-0.5 h-5 w-5 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center text-[10px] text-primary tabular-nums">
                    {i + 1}
                  </span>
                  {rec}
                </li>
