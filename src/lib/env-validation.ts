...
const REQUIRED_VARS: Record<string, string> = {
  ...
  NEXT_PUBLIC_SUPABASE_ANON_KEY: 'Supabase anonymous/public key (Dashboard > Settings > API).',
...
const OPTIONAL_VARS: Record<string, string> = {
  ...
  REWARDFUL_API_SECRET: 'Rewardful (legacy affiliate system) API secret.',
  NEXT_PUBLIC_REWARDFUL_KEY: 'Rewardful public key for client-side snippet.',
  ...
  ADMIN_EMAILS: 'Comma-separated admin email addresses for notification routing.',
}

// ── Deprecated: flag if still set so they can be cleaned up ──────────────────
const DEPRECATED_VARS: Record<string, string> = {
  NEXTAUTH_URL: 'Replaced by NEXT_PUBLIC_SITE_URL in Cycle 1. Remove from .env and hosting config.',
}
...
export function validateEnv() {
  ...
  if (missingOptional.length > 0) {
    console.info(`[env-validation] Optional vars not set: ${missingOptional.join(', ')}`)
  }
}
