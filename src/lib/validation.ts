...
export const essaySchema = z.object({
  ...
  subject: z.enum(['ENGLISH_LANGUAGE', 'ENGLISH_LITERATURE'], {
    message: 'Please select a valid subject',
  }),
  examBoard: z.enum(['AQA', 'EDEXCEL', 'OCR', 'WJEC', 'SQA', 'CCEA', 'CIE', 'OTHER'], {
...
export const cancellationSchema = z.object({
  reason: z.string().max(1000, 'Reason must be 1,000 characters or fewer').optional(),
})

// ─── Safeguarding Report ───────────────────────────────────────────────

export const safeguardingReportSchema = z.object({
  reportType: z.enum(['SELF_HARM', 'ABUSE', 'BULLYING', 'EXPLOITATION', 'OTHER'], {
...
export const breachReportSchema = z.object({
  breachDate: z
    .string()
    .refine((val) => !isNaN(new Date(val).getTime()), { message: 'Please enter a valid date' }),
  discoveryDate: z
    .string()
    .refine((val) => !isNaN(new Date(val).getTime()), { message: 'Please enter a valid date' }),
  breachType: z.enum(
  ...
  dataSubjectsAffected: z.number().int().min(0, 'Must be zero or a positive number'),
  categoriesOfData: z.array(z.string()).min(1, 'Select at least one category of data affected'),
...
export const dsarSchema = z.object({
  ...
  details: z.string().max(2000, 'Details must be 2,000 characters or fewer').optional(),
})
