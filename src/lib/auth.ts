...
export const registrationSchema = z
  ...
  .refine(
    ...
    {
      message: 'You must consent to international data transfer for Qatar users',
  ...
  .refine(
    ...
    {
      message: 'Minors must confirm a parent/guardian is aware of this registration',
...
export const teacherRegistrationSchema = z
  .object({
    ...
    lastName: z.string().min(1, 'Last name is required').max(50, 'Last name is too long'),
    country: z.enum(['UK', 'QA', 'OTHER']),
    schoolName: z.string().max(200, 'School name is too long').optional(),
    schoolEmail: z.string().email('Please enter a valid school email').optional().or(z.literal('')),
    studentCount: z.enum(
      ['1-10', '11-30', '31-60', '61-100', '100+'],
      'Please select how many students you have',
    ),
    subjectTaught: z.string().min(1, 'Subject taught is required').max(100, 'Subject is too long'),
...
export const parentRegistrationSchema = z
  .object({
    ...
    lastName: z.string().min(1, 'Last name is required').max(50, 'Last name is too long'),
    country: z.enum(['UK', 'QA', 'OTHER']),
    studentEmailOrCode: z.string().min(1, "Please enter your child's email or link code"),
...
export function buildConsentRecords(data: z.infer<typeof registrationSchema>): ConsentRecord[] {
  const now = new Date()
