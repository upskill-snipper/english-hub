...
import { parseStudentsCsv, errorsToCsv, STUDENT_COLUMNS } from './csv-parse'
...
describe('parseStudentsCsv', () => {
  ...
  it('handles quoted commas and escaped double quotes inside cells', () => {
    const csv = [HEADER, '"Smith, Jr","O""Brien",smith@example.com,Year 10,10A'].join('\n')
    const result = parseStudentsCsv(csv)
    expect(result.errors).toEqual([])
  ...
  it('emits MISSING_REQUIRED for every empty required cell on a partial row', () => {
    ...
    expect(fields).toEqual(expect.arrayContaining(['lastName', 'yearGroup', 'classCode']))
    expect(result.valid).toEqual([])
  })
  ...
  it('emits MISSING_HEADER errors when required columns are absent', () => {
    ...
    expect(codes).toEqual(expect.arrayContaining(['MISSING_HEADER']))
    expect(result.valid).toEqual([])
  })
