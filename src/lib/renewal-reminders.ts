...
// NOTE: with PRICING.TRIAL_DAYS = 7, this fires on day 5 of the trial. Stripe
  ...
  for (const sub of subscriptions) {
    ...
    try {
      ...
      if (emailResult.success) {
        ...
        result.failed++
        result.errors.push(`Trial ending email failed for user ${sub.userId}: ${emailResult.error}`)
      }
    } catch (err) {
      result.failed++
      const message = err instanceof Error ? err.message : 'Unknown error'
      result.errors.push(`Trial ending reminder error for user ${sub.userId}: ${message}`)
    }
  }
}
...
async function sendReminderAndRecord(
  ...
  try {
    ...
    if (emailResult.success) {
      ...
      result.failed++
      result.errors.push(`Renewal email failed for user ${sub.userId}: ${emailResult.error}`)
    }
  } catch (err) {
    result.failed++
    const message = err instanceof Error ? err.message : 'Unknown error'
    result.errors.push(`Renewal reminder error for user ${sub.userId}: ${message}`)
  }
}
 ...
 */
async function getSubscriptionAmount(stripeSubscriptionId: string | null): Promise<string> {
