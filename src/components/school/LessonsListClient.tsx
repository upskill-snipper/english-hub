...
import type { LessonPlanMetadata } from '@/data/lesson-plans/metadata'
  ...
  return (
    <section className="mb-8">
      <div className="mb-4 flex items-center gap-2">
        ...
        <h2 className="text-lg font-bold text-foreground">Recommended for Your Classes</h2>
      </div>
      <p className="mb-4 text-sm text-muted-foreground">
        Based on analytics from your class performance data, these lessons target identified weak
        areas.
      </p>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {lessons.map((lesson) => {
          const { category, textLabel, difficulty } = lesson
              ...
              <Card className="h-full transition-all duration-200 hover:border-amber-500/30 hover:shadow-md cursor-pointer border-amber-500/10 bg-amber-500/[0.02]">
                <CardContent className="p-5">
                  ...
                  <div className="flex flex-wrap items-center gap-2 mb-3">
                    ...
                    <Badge
                      variant="outline"
                      className={cn('text-[10px]', difficultyColor(difficulty))}
                    >
                      {difficulty}
                    </Badge>
...
export interface LessonsListClientProps {
  lessons: LessonPlanMetadata[]
  recommendedLessons: LessonPlanMetadata[]
  recommendReasons: Record<string, string>
  allSkills: string[]
  allTextLabels: string[]
}
...
export default function LessonsListClient({
  ...
  const filtered = useMemo(() => {
    return lessons.filter((lesson) => {
      // Text/topic filter
      if (textFilter !== 'all' && lesson.textLabel !== textFilter) return false
  ...
  }, [lessons, textFilter, skillFilter, boardFilter, yearFilter, difficultyFilter, searchQuery])
  ...
  const categorised = useMemo(() => {
    const groups: Record<string, LessonPlanMetadata[]> = {}
      ...
      <div className="px-4 py-6 sm:px-6 lg:px-8">
        ...
        {filtered.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground">No lesson plans match your filters.</p>
