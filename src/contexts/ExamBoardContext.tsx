...
const ExamBoardContext = createContext<ExamBoardContextValue | undefined>(
  undefined
);
...
export function ExamBoardProvider({ children }: ExamBoardProviderProps) {
  const [selectedBoard, setSelectedBoardState] = useState<ExamBoard | null>(
    null
  );
...
export function BoardSelector({ forceShow = false, className = '' }: BoardSelectorProps) {
  const { selectedBoard, setSelectedBoard, subject, setSubject } =
    useExamBoard();
