...
| `POST /api/mark/stream`                                    | `ANTHROPIC_MODEL`                   | `src/lib/marking/mark-schemes`                               | `AuditLog` row only                              | **nobody**                                                                                                                                                                                                                                |
...
`/api/mark/stream` is dead. It has no caller in `src/`, `e2e/` or `mobile/`, yet it is deployed, carries the same auth and subscription gates as `/api/mark`, shares its `mark:${user.id}` rate-limit bucket and spends a trial allowance unit. Treat it as live attack surface, not as a feature. It also skips `getExaminerExemplars`, so if it were revived its prompt would silently differ from `/api/mark`.
