...
**Cross-references:** doc 02 (RMS), doc 08 (instructions), doc 09 (deployer/FRIA), doc 10 (transparency), doc 11 (PMM), doc 12 (incidents), doc 16 (roadmap); `business-docs/compliance/rfc/ropa-v1.md` (RoPA — see §9 note).
...
| C5 | "Anthropic zero-retention endpoint + no-training contractual term ensure the disclosure does not enter training pipelines" / "Anthropic enterprise contract specifies no-training; zero-data-retention endpoint used for all child-data paths" (v1 §2 line 32, §4 R3/R6 lines 51,57) | Code calls the standard Anthropic SDK (`src/app/api/essay/feedback/route.ts:62-90`) with **no evidence of a ZDR endpoint configuration**; the contractual ZDR/no-training claim is **unconfirmed in writing**. Public pages assert "not used to train" (`ai-explainer` `:114-119`; `dictionary-legal-long.ts:872`) but the canonical register flags DPA facts as **needing confirmation** (`src/config/subprocessors.ts:27-34`) | **UNSUBSTANTIATED — must be confirmed in writing or claim withdrawn** | Counsel — obtain written Anthropic DPA + ZDR/no-training confirmation (`human-action-checklist.md`); before DoC |
...
| Written Anthropic ZDR/no-training confirmation (R6/R9) | Counsel | Before DoC (`human-action-checklist.md`) |
| B2C human-review UI (R10) | Provider eng | doc 16 roadmap |
| Tone safeguard beyond prompt (R2) | Provider eng | TBD (doc 02) |
| Single-source sub-processor disclosure (R9) | DPO/counsel | Before 2 Aug 2026 (doc 10 §4) |
...
`business-docs/compliance/rfc/ropa-v1.md` (RoPA v1.0) also asserts some of the same unconfirmed controls (e.g. activity 4 "zero-retention terms", "opaque ID strips account linkage at API boundary") and names **Postmark/Trustpilot** where the canonical register names **Resend** (`src/config/subprocessors.ts`). The RoPA must be re-reconciled consistently with this v2.0 DPIA and doc 10 §4. **Owner: DPO; target: before 2 Aug 2026.** The Children's-Code DPIA set (`childrens-code/03-dpias/`) should cross-reference this v2.0 as the authoritative AI-features DPIA.
