...
**P2 items closed since v3:** 5. GAP-8A: Analytics carrying student email -- **CLOSED** (removed from all paths including ClassStudent) 6. GAP-12B: AI opt-out -- **upgraded** from client-only to server-side enforcement
...
5. **Usage capping** -- the enforced cap is the database-backed free and trial allowance metering in `src/lib/usage/**`, plus the Premium subscription gate. The separate 10 essays/day rate limit is configured in code but **not enforced in production** (no shared limiter backend): `business-docs/compliance/controls/rate-limiting-control-status.md`
...
| J.B. Priestley (1894-1984) | Deceased                     | Copyright to 2054 (life+70) | _An Inspector Calls_ (1945)                                        |
...
- Rate limit configured at 10 writes per minute per IP, but **not enforced in production** `business-docs/compliance/controls/rate-limiting-control-status.md`
...
- **Rate limiting (configured, NOT ENFORCED):** Registration (5/hour), AI marking (10/day), age validation (10/15min), parent notification (5/hour), parent data deletion, cookie consent (10/min per IP). These figures are in code, but the limiter has no shared backend configured in production, so none of them holds. Corrected 17 September 2026: `business-docs/compliance/controls/rate-limiting-control-status.md`. Guardian-consent emails are separately throttled by a durable database cooldown and lifetime cap, which does hold.
...
| Rate limiting not enforced in production | Understated previously. It is not only that the in-memory store does not survive a restart: on serverless each instance has its own counter, so the limit does not hold at all. Fix is a Vercel environment change (`business-docs/compliance/controls/rate-limiting-control-status.md`) | P1       |
...
| SEC-02     | Security         | **Rate limiting not enforced in production** (per-instance counters; not merely a restart issue) | Unchanged                | **Re-rated P1.** Now loudly reported at runtime and to Sentry; fix is a Vercel environment change                                      | Founder time (environment variables) |
